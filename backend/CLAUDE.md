...
- **Never assert an exact global count over accumulating shared-tenant state** (the "count-bleed" flake). A `@TestInstance(PER_CLASS)` (or any class that provisions one tenant in `@BeforeAll` and never resets it between methods) accumulates rows from sibling methods — and `processScheduled*`/fan-out/reconciliation paths process *all* of them. So `Mockito.verify(mock, times(repo.findByStatus(X).size()))` or `assertThat(actual).isEqualTo(repo.findAll().size())` is a moving target that passes in isolation and flakes in the full suite (real incident: `AutomationSchedulerScheduledTriggerIntegrationTest` failed CI with `expected: 5L but was: 13L` — multiple accumulated rules each fanned out per active project; PR #1490). Instead: (a) **containment by id** — capture the ids your test seeded and assert `.contains(seededA, seededB).doesNotContain(excluded)`; (b) **baseline + delta** — `long before = repo.count(); … assertThat(repo.count() - before)`; or (c) **id-scoped query** — filter by the entity created in *this* test. Do NOT "fix" such a flake by re-tightening to another exact count, and do NOT use AssertJ `containsExactlyInAnyOrder` over shared-mock invocations whose ids can recur across accumulated rules — both reintroduce the flake.
