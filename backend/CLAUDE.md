...
**⚠️ Known violations:** Several existing controllers (PortalAuthController, ReportingController, DocumentController, DataRequestController, DashboardController, OrgSettingsController, RetainerAgreementController, MockPaymentController, PortalBrandingController, PortalDigestInternalController) predate this rule enforcement. Do NOT use them as reference patterns. All new controllers must follow this discipline. Cleanup tracked in `documentation/tech-debt.md` TD-009 (the latter three were surfaced 2026-05-02 when the ArchUnit upgrade made the rule actually enforce).
