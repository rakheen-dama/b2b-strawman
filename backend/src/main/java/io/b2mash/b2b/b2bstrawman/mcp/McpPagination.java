...
public final class McpPagination {
  ...
  /**
   * Hard per-call item ceiling. A tool whose underlying result set exceeds this should return
   * {@link McpError#responseTooLarge()} rather than a truncated blob, so the LLM is told to narrow
   * its query instead of silently losing rows.
   */
  public static final int RESPONSE_ITEM_CEILING = AUDIT_MAX_SIZE;
