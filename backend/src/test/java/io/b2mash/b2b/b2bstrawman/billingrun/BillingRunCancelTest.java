...
class BillingRunCancelTest {
  ...
  void setUp() throws Exception {
    ...
    memberIdOwner =
        UUID.fromString(
            syncMember("user_cancel_owner", "cancel_owner@test.com", "Cancel Owner", "owner"));
  ...
  void cancelPreview_deletesRunAndItems() throws Exception {
    ...
    mockMvc
        .perform(get("/api/billing-runs/" + runId).with(ownerJwt()))
  ...
  private JwtRequestPostProcessor ownerJwt() {
    return jwt()
        .jwt(j -> j.subject("user_cancel_owner").claim("o", Map.of("id", ORG_ID, "rol", "owner")))
