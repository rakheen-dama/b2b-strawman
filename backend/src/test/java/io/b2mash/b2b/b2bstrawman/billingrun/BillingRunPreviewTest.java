...
class BillingRunPreviewTest {
  ...
  void setUp() throws Exception {
    ...
    TestModuleHelper.enableModules(mockMvc, ORG_ID, "user_preview_owner", "bulk_billing");
  ...
  void loadPreview_createsEntrySelectionRecords() throws Exception {
    ...
    var itemsResult =
        mockMvc
            .perform(
                get("/api/billing-runs/" + billingRunId + "/items")
                    .with(TestJwtFactory.ownerJwt(ORG_ID, "user_preview_owner")))
            .andExpect(status().isOk())
  ...
  void loadPreview_prerequisiteFailure_autoExcludes() throws Exception {
    ...
    var itemsResult =
        mockMvc
            .perform(
                get("/api/billing-runs/" + prereqRunId + "/items")
                    .with(TestJwtFactory.ownerJwt(ORG_ID, "user_preview_owner")))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.length()").value(2))
            .andReturn();
