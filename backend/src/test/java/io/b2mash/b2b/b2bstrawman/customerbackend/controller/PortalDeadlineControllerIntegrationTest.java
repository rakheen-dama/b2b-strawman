...
import io.b2mash.b2b.b2bstrawman.customerbackend.controller.PortalDeadlineControllerIntegrationTest.ModuleGuardTestConfig;
...
/**
 * Integration tests for {@link PortalDeadlineController} (Epic 497A). Covers the scenarios
 * specified in task 497.7 plus review-driven additions:
 *
 * <ol>
 ...
 *   <li>Tenant without the {@code deadlines} module → 404 on the list endpoint with a
 *       deadline-specific ProblemDetail (ADR-254: portal hides disabled modules).
 *   <li>Detail endpoint resolves a row by {@code (sourceEntity, id)}.
 *   <li>Detail endpoint also returns 404 when the {@code deadlines} module is disabled.
 *   <li>Detail endpoint enforces per-customer scoping — a portal contact scoped to customer A
 *       cannot read customer B's deadline by guessing {@code (sourceEntity, id)} (ADR-253).
 * </ol>
 *
 * <p>The {@link VerticalModuleGuard} mock is declared once in a {@link TestConfiguration} rather
 * than via {@code @MockitoBean} on the test class — per backend/CLAUDE.md, sharing a single mock
 * definition across tests avoids context-cache churn.
 */
@SpringBootTest
@AutoConfigureMockMvc
@Import({TestcontainersConfiguration.class, ModuleGuardTestConfig.class})
@ActiveProfiles("test")
...
class PortalDeadlineControllerIntegrationTest {
  ...
  // Second customer in the same tenant used to exercise cross-customer isolation on the detail
  // endpoint. The portal schema is shared across customers and is keyed on customer_id; a missing
  // scope check would let customer A pull customer B's deadline by guessing a UUID.
  private UUID otherCustomerId;
  private UUID otherCustomerDeadlineId;
  ...
  /**
   * Extracts the {@link VerticalModuleGuard} mock into a shared {@code @TestConfiguration}.
   * Multiple portal-controller integration tests need the same mock; declaring it once here avoids
   * a distinct Spring context cache key per test and keeps this test within the CLAUDE.md rule on
   * {@code @MockitoBean} overuse.
   */
  @TestConfiguration
  static class ModuleGuardTestConfig {
    @Bean
    @Primary
    VerticalModuleGuard moduleGuard() {
      return Mockito.mock(VerticalModuleGuard.class);
    }
  }
  ...
  void setup() throws Exception {
        ...
        .run(
            () -> {
              ...
              var otherCustomer =
                  customerRepository.save(
                      TestCustomerFactory.createActiveCustomer(
                          "Other Deadline Customer", "other-deadline@test.com", legalMemberId));
    ...
    // Seed a deadline owned by the OTHER customer so the cross-customer isolation test has
    // something to try to read with the legal portal token.
    otherCustomerDeadlineId = UUID.randomUUID();
    deadlineRepo.upsert(
        new PortalDeadlineView(
            otherCustomerDeadlineId,
            "FILING_SCHEDULE",
            otherCustomerId,
            null,
            "FILING",
            "Other customer filing",
            today.plusDays(20),
            "UPCOMING",
  ...
  // ==========================================================================
  // Scenario 5 — detail endpoint also 404s when module disabled
  // ==========================================================================

  @Test
  void get_detailEndpointReturns404WhenDeadlinesModuleIsDisabled() throws Exception {
    when(moduleGuard.isModuleEnabled(anyString())).thenReturn(false);
  ...
  // ==========================================================================
  // Scenario 6 — cross-customer isolation on the detail endpoint (ADR-253)
  // ==========================================================================

  @Test
  void get_detailEndpointCannotReadAnotherCustomersDeadline() throws Exception {
    when(moduleGuard.isModuleEnabled(DEADLINES_MODULE)).thenReturn(true);

    // Direct hit on the OTHER customer's deadline using the legal customer's portal token must
    // 404, not 200 — the portal schema is keyed on customer_id and a portal contact scoped to
    // legalCustomerId must never surface another customer's row.
    mockMvc
        .perform(
            get("/portal/deadlines/{sourceEntity}/{id}", "FILING_SCHEDULE", otherCustomerDeadlineId)
                .header("Authorization", "Bearer " + legalPortalToken))
        .andExpect(status().isNotFound());
  }
}
