...
class ProjectIntegrationTest {
  ...
  void provisionTenants() throws Exception {
    ...
    customRoleMemberId =
        UUID.fromString(
            syncMember(
                ORG_ID, "user_proj_314b_custom", "proj_custom@test.com", "Custom User", "member"));
    noCapMemberId =
        UUID.fromString(
            syncMember(
                ORG_ID, "user_proj_314b_nocap", "proj_nocap@test.com", "NoCap User", "member"));
  ...
  void customRoleWithCapability_accessesProjectEndpoint_returns200() throws Exception {
    ...
    mockMvc
        .perform(get("/api/projects/" + id + "/unbilled-summary").with(customRoleJwt()))
        .andExpect(status().isOk());
  }
  ...
  void customRoleWithoutCapability_accessesProjectEndpoint_returns403() throws Exception {
    ...
    mockMvc
        .perform(get("/api/projects/" + id + "/unbilled-summary").with(noCapabilityJwt()))
        .andExpect(status().isForbidden());
  }
  ...
  private JwtRequestPostProcessor customRoleJwt() {
    return jwt()
        .jwt(
            j ->
                j.subject("user_proj_314b_custom")
  ...
  private JwtRequestPostProcessor noCapabilityJwt() {
    return jwt()
        .jwt(
            j ->
                j.subject("user_proj_314b_nocap").claim("o", Map.of("id", ORG_ID, "rol", "member")))
        .authorities(List.of(new SimpleGrantedAuthority("ROLE_ORG_MEMBER")));
  }
}
