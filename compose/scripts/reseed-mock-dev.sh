#!/usr/bin/env bash
# reseed-mock-dev.sh — Re-run the seed container to reset E2E data.
...
#
# Usage: bash compose/scripts/reseed-mock-dev.sh
set -euo pipefail
...
if ! curl -sf http://localhost:8081/actuator/health > /dev/null 2>&1; then
  ...
  echo "Start the stack first: bash compose/scripts/start-mock-dev.sh"
  exit 1
fi
...
docker compose -p e2e -f "$COMPOSE_FILE" rm -f seed 2>/dev/null || true

# Run seed again
docker compose -p e2e -f "$COMPOSE_FILE" up seed
