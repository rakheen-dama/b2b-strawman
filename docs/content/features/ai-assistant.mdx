...
Each conversation starts fresh when you open the assistant panel. Within a session, the assistant remembers your previous messages and its own responses, so you can have a natural back-and-forth conversation. Ask a follow-up question without repeating context, and the assistant will understand.

However, conversation history is not saved between sessions. If you close the panel or refresh the page, the conversation resets. This keeps interactions lightweight and ensures no conversation data is stored beyond the active session.

## Limitations

- **Session-scoped history** -- conversations are not persisted. Closing the panel or refreshing the page starts a new conversation.
...
- **Read-only data access** -- the assistant can look up your data to answer questions, but it does not modify records on your behalf (unless confirmed via a confirmation card).
