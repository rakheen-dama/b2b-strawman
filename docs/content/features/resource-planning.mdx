...
Click **Save**. The cell updates with the new hours and a coloured project bar. If the total exceeds capacity, the cell turns red and an over-allocation warning appears.
</Steps>

<Callout type="warning">
  Over-allocating a team member does not block the save — HeyKazi displays an amber warning banner so you can make a conscious decision about the trade-off.
</Callout>
...
<Callout type="info">
  Utilisation measures the percentage of total tracked time that is billable. Industry benchmarks typically range from 60% to 80%.
</Callout>
