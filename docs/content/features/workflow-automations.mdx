...
Click **Add Action** and select an action type. Fill in the action-specific fields, insert variables where needed, and optionally set a delay. Repeat to add multiple actions — drag to reorder if the sequence matters.
...
The execution log provides a complete audit trail of every automation run. Filter by **rule** and **status** (Running, Completed, Failed, or Skipped) to find specific executions. The table shows the rule name, trigger type, timestamp, status badge, whether conditions were met, the number of actions executed, and the duration.
