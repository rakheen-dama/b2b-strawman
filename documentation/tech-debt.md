...
### TD-009: Pre-existing controller→repository violations surfaced by TD-008 fix — RESOLVED (2026-06-11)
...
**Resolved**: 2026-06-11 (PR `refactor/controller-repo-injection`). The two enforceable repo-injection violations were cleared and `MockPaymentController` was converted to a permanent justified exemption. The ArchUnit rule now enforces against the formerly-exempt controllers.

**Original file list and resolution** (the three controllers the rule's exclusion list carried):

- `MockPaymentController` (`integration/payment/`) — injected `InvoiceRepository`, `OrgSchemaMappingRepository`. **PERMANENT EXEMPTION** (not refactored): dev/test-only PSP-checkout harness, `@Profile({"local","dev","keycloak"})`-gated, never reachable in prod. It performs a cross-tenant invoice search (find-which-schema-owns-an-invoice) that has no production analogue; extracting a service layer for throwaway dev tooling adds indirection without value. The exclusion entry in `LayerDependencyRulesTest` was restructured into an explicit, commented allowlist of one.
- `PortalBrandingController` (`portal/`) — injected `OrgSchemaMappingRepository`, `OrganizationRepository`, `OrgSettingsRepository`. **FIXED**: repository access + logo-URL generation + cross-schema branding resolution moved into a new `PortalBrandingService`; the controller is now pure delegation. Exclusion entry **deleted** — the rule enforces against it again.
- `PortalDigestInternalController` (`portal/notification/`) — the original TD-009 note claimed it "injects repository types", **but this was inaccurate**: it only injects `PortalDigestScheduler` and delegates one endpoint to it (no `*Repository` dependency, confirmed by running the rule with all three exemptions removed — only `MockPaymentController` and `PortalBrandingController` fired). Its exclusion entry was dead code; **deleted**, no source change needed.

(Pre-existing exemptions for `DevPortalController`, `InternalAuditController`, `PaymentWebhookController` remain on the rule's exclusion list — out of scope for TD-009, which covered only the repo-injection trio.)
...
- `OrgSettingsController` (2026-06-11): the "configuration sprawl" flagged for this controller was **not** behavioural logic — every one of its 14 endpoints was already pure single-method delegation (no conditionals/helpers/transforms/orchestration, no repository injection). The actual smell was 11 request-DTO records nested inside the controller, one of which (`DataProtectionSettingsRequest`) was imported *back* by `OrgSettingsService` (a service→controller backwards dependency). Relocated all 11 request DTOs into the existing `settings/dto` package (matching `SettingsResponse`/`UpdateModulesRequest`) and repointed the service import; the controller now holds only its delegating endpoint methods. Behaviour-preserving compile-time move — the validation paths it carries are declarative bean-validation already exercised end-to-end by `OrgSettingsIntegrationTest` (e.g. `putSettings_rejectsTwoCharCurrency`, `patchTaxSettings_validatesMaxLengths`), so no new characterization test was warranted. Removed from the `backend/CLAUDE.md` "Known violations" list. No ArchUnit exclusion entry existed (it never injected a repository), so no rule change was needed. The `OrgSettings` entity / service restructure remains deferred to Wave 3.3 and was deliberately left untouched.
- `refactor/controller-repo-injection` (2026-06-11, slice 5 — **closes out the TD-009 enforceable core**): `PortalBrandingController` data access (3 repositories + storage + read-only tx) moved into a new `PortalBrandingService`; controller is pure delegation. The two repo-injection exclusion entries (`PortalBrandingController`, `PortalDigestInternalController`) were deleted from `LayerDependencyRulesTest`; `MockPaymentController` retained as a single, commented, permanently-justified allowlist entry. The `callForTenant` (shard-unaware) call moved with the branding logic, so the D5 grandfathered entry in `TenantScopeBindingTest` was retargeted `PortalBrandingController` → `PortalBrandingService` (see TD-D5 below — debt relocated, not added). Characterization test added for the previously-unasserted logo-URL-generation-failure branch (`PortalBrandingControllerIntegrationTest#branding_logo_url_generation_failure_returns_null_logoUrl`).

**Trigger to fix**: TD-009's repo-injection core is closed. The remaining `backend/CLAUDE.md` "Known violations" entries (thin-controller discipline beyond repo injection — `PortalAuthController`, `ReportingController`, etc.) are opportunistic clean-ups for when those controllers are next touched.
...
- **Need a repository lookup / minor refactor**: `MemberSyncService`, `EmailWebhookService`, `UnsubscribeService`, `AcceptanceService` (portal `AcceptanceContext` lacks shardId), `PackInstallService`, `PackReconciliationRunner`, `InternalAuditController`, `AuditExportService`, `CustomerAuthFilter`, `PortalBrandingService` (the `callForTenant` call moved here from `PortalBrandingController` in the 2026-06-11 TD-009 refactor), and the portal sync services (`DeadlinePortalSyncService`, `PortalResyncService`, `RetainerPortalSyncService`, `TrustLedgerPortalSyncService`). Each can inject `OrgSchemaMappingRepository` (most already do) and look up `getShardId()` at the call site, or carry shardId on the relevant context object.
