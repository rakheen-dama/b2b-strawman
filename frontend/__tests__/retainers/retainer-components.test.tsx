...
describe("RetainerStatusBadge", () => {
  ...
  it("renders correct variant for each status", () => {
    ...
    expect(screen.getByText("Terminated")).toHaveAttribute("data-variant", "neutral");
  });
});
...
describe("RetainerProgress", () => {
  ...
  it("shows correct percentage and color changes at thresholds", () => {
    ...
    const { unmount: u1 } = render(
      <RetainerProgress type="HOUR_BANK" consumedHours={20} allocatedHours={40} />
    );
    ...
    const { unmount: u2 } = render(
      <RetainerProgress type="HOUR_BANK" consumedHours={35} allocatedHours={40} />
    );
    ...
    const { unmount: u3 } = render(
      <RetainerProgress type="HOUR_BANK" consumedHours={45} allocatedHours={40} />
    );
    expect(screen.getByTestId("progress-bar")).toHaveClass("bg-red-500");
    u3();

    // FIXED_FEE shows consumed only, no bar
    render(<RetainerProgress type="FIXED_FEE" consumedHours={10} allocatedHours={null} />);
