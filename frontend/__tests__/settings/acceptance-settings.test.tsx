...
describe("AcceptanceSettingsForm", () => {
  ...
  it("renders acceptance expiry input with current value", () => {
    render(<AcceptanceSettingsForm slug="acme" acceptanceExpiryDays={45} />);
    const input = screen.getByRole("spinbutton");
    expect(input).toHaveValue(45);
    expect(
      screen.getByText(/Number of days before an acceptance request expires/)
    ).toBeInTheDocument();
  });

  it("saves updated expiry days", async () => {
    const { updateAcceptanceSettings } =
      await import("@/app/(app)/org/[slug]/settings/acceptance/actions");
    const user = userEvent.setup();
    render(<AcceptanceSettingsForm slug="acme" acceptanceExpiryDays={30} />);
