...
vi.mock("@/app/(app)/org/[slug]/settings/general/profile-actions", () => ({
  updateVerticalProfile: (...args: unknown[]) => mockUpdateVerticalProfile(...args),
...
describe("VerticalProfileSection — owner view", () => {
  it("renders dropdown and Apply Profile button for owner", () => {
    mockSWRLoaded();
    render(<VerticalProfileSection slug="acme" currentProfile={null} isOwner={true} />);
    // Select trigger is present (owner sees dropdown)
    expect(screen.getByRole("combobox")).toBeInTheDocument();
    // Apply Profile button is present
    expect(screen.getByRole("button", { name: /Apply Profile/i })).toBeInTheDocument();
  });

  it("shows read-only text for non-owner", () => {
    render(<VerticalProfileSection slug="acme" currentProfile="legal-za" isOwner={false} />);
    // No combobox/dropdown
    expect(screen.queryByRole("combobox")).not.toBeInTheDocument();
    // No Apply Profile button
    expect(screen.queryByRole("button", { name: /Apply Profile/i })).not.toBeInTheDocument();
    // Profile name shown
...
describe("VerticalProfileSection — confirmation dialog", () => {
  it("confirmation dialog appears after selecting profile and clicking Apply Profile", async () => {
    ...
    expect(
      screen.getByText(/Changing your vertical profile will add new field definitions/i)
    ).toBeInTheDocument();
  });
  ...
  it("calls updateVerticalProfile on confirm", async () => {
    ...
    expect(mockUpdateVerticalProfile).toHaveBeenCalledWith("acme", "legal-za");
  });
});
