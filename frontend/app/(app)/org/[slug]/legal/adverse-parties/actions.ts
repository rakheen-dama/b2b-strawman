...
import type { AdverseParty, AdversePartyLink, AdversePartyType } from "@/lib/types";
...
export async function fetchAdverseParties(
  ...
  return api.get<PaginatedResponse<AdverseParty>>(`/api/adverse-parties?${params.toString()}`);
}

export async function fetchAdverseParty(id: string): Promise<AdversePartyDetail> {
...
export async function createAdverseParty(
  ...
  try {
    ...
    const message = error instanceof ApiError ? error.message : "Failed to create adverse party";
...
export async function updateAdverseParty(
  ...
  try {
    ...
    const message = error instanceof ApiError ? error.message : "Failed to update adverse party";
    return { success: false, error: message };
  }
}

export async function deleteAdverseParty(slug: string, id: string): Promise<ActionResult> {
  try {
    await api.delete(`/api/adverse-parties/${id}`);
    revalidatePath(`/org/${slug}/legal/adverse-parties`);
    return { success: true };
  } catch (error) {
    const message = error instanceof ApiError ? error.message : "Failed to delete adverse party";
    return { success: false, error: message };
  }
}
...
export async function linkAdverseParty(
  ...
  try {
    ...
    const message = error instanceof ApiError ? error.message : "Failed to link adverse party";
    return { success: false, error: message };
  }
}

export async function unlinkAdverseParty(slug: string, linkId: string): Promise<ActionResult> {
  try {
    await api.delete(`/api/adverse-party-links/${linkId}`);
    revalidatePath(`/org/${slug}/legal/adverse-parties`);
    return { success: true };
  } catch (error) {
    const message = error instanceof ApiError ? error.message : "Failed to unlink adverse party";
    return { success: false, error: message };
  }
}

export async function fetchProjectAdverseParties(projectId: string): Promise<AdversePartyLink[]> {
  return api.get<AdversePartyLink[]>(`/api/projects/${projectId}/adverse-parties`);
}
