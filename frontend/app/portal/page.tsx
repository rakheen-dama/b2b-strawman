...
function PortalLoginContent() {
  ...
  const handleRequestLink = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        const result = await portalApi.post<MagicLinkResponse>("/portal/auth/request-link", {
          email,
          orgId: orgSlug,
        });
        ...
        if (err instanceof PortalApiError) {
          setError(
            err.message || "Failed to send magic link. Please check your email and organization."
          );
        } else {
  ...
  const handleExchangeToken = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        ...
        const result = await portalApi.post<PortalAuthResponse>("/portal/auth/exchange", {
          token: tokenValue,
          orgId: exchangeOrgId,
        });
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-50 dark:bg-slate-950">
      <div className="w-full max-w-md space-y-8 px-6">
        ...
        <div className="overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
          {/* Brand color stripe */}
          {branding?.brandColor && (
            <div className="h-1" style={{ backgroundColor: branding.brandColor }} />
          )}
          <div className="p-8">
            {step === "email" && (
              ...
              <form onSubmit={handleExchangeToken} className="space-y-4">
                <div className="flex flex-col items-center gap-3 text-center">
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-400">
                    For the MVP, the magic link is shown below. In production, this would be sent to
                    your email.
                  </p>
                </div>
                ...
                {magicLink && (
                  <div className="space-y-2">
                    ...
                    <div className="rounded-lg bg-slate-50 p-3 font-mono text-xs break-all text-slate-700 dark:bg-slate-800 dark:text-slate-300">
                      {magicLink}
                    </div>
                  </div>
                )}
