...
import {
  initiateCustomerUpload,
  confirmCustomerUpload,
  cancelCustomerUpload,
...
export function CustomerDocumentUpload({ slug, customerId }: CustomerDocumentUploadProps) {
  ...
  const processUpload = useCallback(
    async (item: UploadItem) => {
      ...
      const initResult = await initiateCustomerUpload(
        slug,
        customerId,
        item.file.name,
        contentType,
      ...
      if (!uploadSuccess) {
        if (initResult.documentId) await cancelCustomerUpload(initResult.documentId);
        return;
      }

      // Step 3: Confirm
      dispatch({ type: "SET_STATUS", id: item.id, status: "confirming" });
      const confirmResult = await confirmCustomerUpload(slug, customerId, initResult.documentId);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload files associated with this customer. Documents default to internal visibility.
          </DialogDescription>
        </DialogHeader>
