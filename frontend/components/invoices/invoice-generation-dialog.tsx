      ...
      <Dialog open={h.open} onOpenChange={h.handleOpenChange}>
        ...
        {h.step === 1 && (
          <div className="space-y-4">
            ...
            <DialogFooter>
              <Button type="button" variant="plain" onClick={() => h.handleOpenChange(false)} disabled={h.isPending}>
                Cancel
              </Button>
              <Button type="button" onClick={h.handleFetchUnbilled} disabled={h.isPending || h.currency.length !== 3}>
        ...
        {h.step === 2 && h.unbilledData && (
          <div className="space-y-4">
            <Button type="button" variant="ghost" size="sm" className="text-slate-600 dark:text-slate-400" onClick={() => h.setStep(1)}>
            ...
            {h.nullRateEntries.length > 0 && (
              <div className="rounded-md border border-amber-200 bg-amber-50 p-3 dark:border-amber-800 dark:bg-amber-900/20" data-testid="null-rate-warning">
                <p className="mb-1.5 text-sm font-medium text-amber-800 dark:text-amber-200">
                  {h.nullRateEntries.length} time {h.nullRateEntries.length === 1 ? "entry has" : "entries have"} no rate card
                </p>
                <ul className="space-y-0.5">
                  {h.nullRateEntries.map((e) => (
                    <li key={e.id} className="text-xs text-amber-700 dark:text-amber-300">
                      {e.memberName}, {formatDuration(e.durationMinutes)} on {formatDate(e.date)} ({e.projectName} / {e.taskTitle})
                    </li>
                  ))}
                </ul>
              </div>
            ...
            {h.validationChecks && h.validationChecks.length > 0 && (
              <div data-testid="validation-checklist" className="rounded-lg border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-900/50">
                <p className="mb-2 text-sm font-medium text-slate-700 dark:text-slate-300">Pre-generation checks</p>
                <ul className="space-y-1">
                  {h.validationChecks.map((check, idx) => (
                    <li key={idx} className="flex items-center gap-2 text-sm">
                      ...
                      <span className={check.passed ? "text-slate-700 dark:text-slate-300" : "text-yellow-800 dark:text-yellow-200"}>
                        {check.message}
                      </span>
                    </li>
                  ))}
            ...
            <DialogFooter>
              ...
              {!h.validationChecks ? (
                <Button type="button" onClick={h.handleRunValidation} disabled={h.isPending || h.isValidating || h.totalItemCount === 0}>
                  {h.isValidating ? "Validating..." : "Validate & Create Draft"}
                </Button>
              ) : (
                <Button type="button" onClick={h.handleCreateDraft} disabled={h.isPending || h.totalItemCount === 0}>
                  {h.isPending ? "Creating..." : h.validationChecks.some((c) => !c.passed)
...
function ExpenseSelectionSection({
  ...
  const expensesByProject: Record<string, { projectName: string; expenses: UnbilledExpenseEntry[] }> = {};
  ...
  return (
    <div data-testid="expense-selection-section">
      <div className="mb-2 flex items-center justify-between">
        ...
        <button type="button" onClick={onToggleAll} className="text-xs text-teal-600 hover:text-teal-700 dark:text-teal-400" disabled={selectableExpenses.length === 0}>
          {allSelected ? "Deselect All" : "Select All Expenses"}
        </button>
      </div>
      <div className="max-h-60 space-y-3 overflow-y-auto">
        {Object.entries(expensesByProject).map(([projectId, { projectName, expenses: projectExpenses }]) => (
          <div key={projectId} className="rounded-lg border border-slate-200 dark:border-slate-800">
            <div className="border-b border-slate-200 px-4 py-2 dark:border-slate-800">
              <span className="text-sm font-medium text-slate-900 dark:text-slate-100">{projectName}</span>
            </div>
            <div className="divide-y divide-slate-100 dark:divide-slate-800/50">
              {projectExpenses.map((expense) => {
                const currencyMismatch = expense.currency !== currency;
                return (
                  <label key={expense.id} className={`flex items-center gap-3 px-4 py-2 text-sm ${currencyMismatch ? "cursor-not-allowed opacity-50" : "cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-900/50"}`}>
                    <input type="checkbox" checked={selectedExpenseIds.has(expense.id)} onChange={() => onToggleExpense(expense.id)} disabled={currencyMismatch} className="size-4 rounded accent-teal-600" />
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <span className="font-medium text-slate-900 dark:text-slate-100">{expense.description}</span>
                        <ExpenseCategoryBadge category={expense.category} />
                        {currencyMismatch && <span className="text-xs text-slate-400">({expense.currency})</span>}
                      </div>
                      <div className="text-xs text-slate-500">
                        {formatDate(expense.date)}
                        {expense.markupPercent != null && <> &middot; {expense.markupPercent}% markup</>}
                      </div>
                    </div>
...
function EntryRow({
  ...
  return (
    <label className={`flex items-center gap-3 px-4 py-2 text-sm ${currencyMismatch ? "cursor-not-allowed opacity-50" : "cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-900/50"}`}>
      <input type="checkbox" checked={isSelected} onChange={() => onToggle(entry.id)} disabled={currencyMismatch} className="size-4 rounded accent-teal-600" />
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <span className="font-medium text-slate-900 dark:text-slate-100">{entry.taskTitle}</span>
          {currencyMismatch && <span className="text-xs text-slate-400">({entry.billingRateCurrency})</span>}
        </div>
        <div className="text-xs text-slate-500">
          {entry.memberName} &middot; {formatDate(entry.date)} &middot; {formatDuration(entry.durationMinutes)}
        </div>
      </div>
      <span className="shrink-0 text-right font-medium text-slate-700 dark:text-slate-300">
