...
import { ChevronDown, ChevronRight, Download, FileText, Send, Trash2 } from "lucide-react";
...
export function GeneratedDocumentsList({
  ...
  async function handleDocxDownload(doc: GeneratedDocumentListResponse) {
    setError(null);
    try {
      const result = await downloadDocxGeneratedDocumentAction(doc.id);
      if (result.success && result.docxBase64) {
        ...
        setError(result.error ?? "Failed to download DOCX document.");
      }
    } catch {
      setError("Failed to download DOCX document.");
    }
  }
      ...
      <Table>
        ...
        <TableBody>
          {documents.map((doc) => {
            ...
            const columnCount = 6 + (showAcceptanceColumn ? 1 : 0);
            const format = doc.outputFormat || "PDF";
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      {hasDocx && (
                        <Button
                          ...
                          size="icon"
                          className="size-8"
                          onClick={() => handleDocxDownload(doc)}
                          title="Download DOCX"
                        >
