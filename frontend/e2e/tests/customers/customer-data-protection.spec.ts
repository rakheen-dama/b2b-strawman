...
test.describe('DP-01: Customer Data Protection', () => {
  ...
  test('Download customer data export', async ({ page }) => {
    ...
    const customerLink = page.getByRole('link').filter({ hasText: /Kgosi|Naledi|Vukani|Moroka/i }).first()
    ...
    const downloadBtn = page.getByRole('button', { name: /download.*data|export.*data|data.*export/i }).first()
    ...
    if (!found && await moreMenu.isVisible().catch(() => false)) {
      ...
      const downloadMenuItem = page.getByRole('menuitem', { name: /download.*data|export.*data/i }).first()
  ...
  test('Anonymize customer preview shows entity counts', async ({ page }) => {
    ...
    const anonBtn = page.getByRole('button', { name: /anonymi|delete.*personal|remove.*data/i }).first()
    const moreMenu = page.getByRole('button', { name: /more|actions|menu/i }).first()
    ...
    if (!found && await moreMenu.isVisible().catch(() => false)) {
      await moreMenu.click()
      await page.waitForTimeout(500)
      const anonMenuItem = page.getByRole('menuitem', { name: /anonymi|delete.*personal/i }).first()
  ...
  test('Execute anonymization', async ({ page }) => {
    ...
    const anonBtn = page.getByRole('button', { name: /anonymi|delete.*personal/i }).first()
    ...
    const confirmInput = page.getByRole('textbox', { name: /confirm|customer.*name|type.*name/i }).first()
