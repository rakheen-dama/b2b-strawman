...
test.describe("DOC-03: Document Acceptance", () => {
  test("Send document for acceptance", async ({ page }) => {
    ...
    const activeCustomer = customers.find(
      (c: { lifecycleStatus: string; id: string }) => c.lifecycleStatus === "ACTIVE"
    );
    ...
    const generateButton = page
      .getByRole("button", { name: /Generate Document|Send.*Document|Generate/i })
      .first();
  ...
  test("Portal contact views document", async ({ page }) => {
    ...
    const hasDoc = await page
      .getByText(/document|letter|engagement/i)
      .first()
    ...
    if (hasDoc) {
      ...
      await page
        .getByRole("link")
        .filter({ hasText: /document|letter|engagement/i })
        .first()
        .click();
      await page.waitForLoadState("networkidle");
      await expect(page.locator("body")).not.toContainText("Something went wrong");
  ...
  test("Portal contact accepts document", async ({ page }) => {
    ...
    const hasAccepted = await page
      .getByText(/accepted|confirmed|success/i)
      .first()
      .isVisible({ timeout: 5000 })
      .catch(() => false);
  ...
  test("Firm sees acceptance metadata", async ({ page }) => {
    ...
    await page.goto(`${BASE}/documents`);
    const hasDocumentsPage = await page
      .getByRole("heading", { name: /Documents/i, level: 1 })
      .isVisible({ timeout: 5000 })
      .catch(() => false);
    if (!hasDocumentsPage) {
      test.skip(
        true,
        "Standalone documents page not available — generated docs accessible via entity pages"
      );
      return;
    }
