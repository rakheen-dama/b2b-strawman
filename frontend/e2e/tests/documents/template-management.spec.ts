...
test.describe('DOC-01: Template Management', () => {
  ...
  test('Template list shows seeded templates', async ({ page }) => {
    ...
    const hasEmptyState = await page.getByText(/No templates/i).isVisible({ timeout: 3000 }).catch(() => false)
    const hasTemplateNames = await page.getByText(/Engagement Letter|Statement of Work|Cover Letter|NDA|Project Summary/i).first().isVisible({ timeout: 3000 }).catch(() => false)
    ...
    const newTemplateButton = page.getByRole('link', { name: /New Template/i })
  ...
  test('Edit template content', async ({ page }) => {
    ...
    const hasEditor = await page.locator('.ProseMirror, .tiptap, [contenteditable]').first().isVisible({ timeout: 3000 }).catch(() => false)
    const hasFormInputs = await page.getByLabel(/Name|Content/i).first().isVisible({ timeout: 3000 }).catch(() => false)
