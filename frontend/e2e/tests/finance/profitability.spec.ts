...
test.describe("FIN-01: Profitability Dashboard", () => {
  ...
  test("Project profitability view", async ({ page }) => {
    ...
    const projectTab = page
      .locator(
        '[role="tab"]:has-text("Project"), button:has-text("Project"), a:has-text("Project")'
      )
      .first();
  ...
  test("Customer profitability view", async ({ page }) => {
    ...
    const customerTab = page
      .locator(
        '[role="tab"]:has-text("Customer"), button:has-text("Customer"), a:has-text("Customer")'
      )
      .first();
  ...
  test("Utilization metrics", async ({ page }) => {
    ...
    const utilTab = page
      .locator(
        '[role="tab"]:has-text("Utilization"), button:has-text("Utilization"), a:has-text("Utilization")'
      )
      .first();
