...
test.describe.serial('Keycloak Member Invite & RBAC', () => {
  test.skip(
    !hasState(),
    'Requires onboarding.spec.ts to run first (state file missing)'
  )
  ...
  test('7. RBAC: Member (Carol) cannot access financial settings', async ({
    page,
  }) => {
    ...
    await expect(page.locator('body')).toContainText(
      /don.t have access|permission/i
    )
