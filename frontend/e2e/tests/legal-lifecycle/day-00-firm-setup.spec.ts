...
test.describe.serial("Day 0 — Firm Setup", () => {
  ...
  test("Alice: Legal-za profile active — sidebar shows legal terms", async ({ page }) => {
    ...
    const legalNavPresent = updatedText?.match(/matters/i) || updatedText?.match(/clients/i);
    if (!legalNavPresent) {
  ...
  test("Alice: Settings — set brand colour #1B3A4B", async ({ page }) => {
    ...
    const colorInput = page
      .locator(
        'input[type="color"], input[name*="brand"], input[name*="colour"], input[name*="color"]'
      )
      .first();
  ...
  test("Alice: Settings — verify billing rate creation flow", async ({ page }) => {
    ...
    const addRateBtn = page
      .getByRole("button", { name: /new.*(rate|tariff)|add.*(rate|tariff)/i })
      .first();
    ...
    if (hasDialog) {
      ...
      const amountField = page
        .getByRole("textbox", { name: /amount|rate/i })
        .or(page.getByRole("spinbutton", { name: /amount|rate/i }))
        .first();
  ...
  test("Alice: Settings — create cost rates for Alice R1000, Bob R500, Carol R200", async ({
    page,
  }) => {
    await loginAs(page, "alice");
    await page.goto(`${BASE}/settings/rates`);
  ...
  test("Alice: Settings — create VAT 15%", async ({ page }) => {
    ...
    if (hasAddTax) {
      ...
      const rateField = page
        .getByRole("textbox", { name: /rate|percentage/i })
        .or(page.getByRole("spinbutton", { name: /rate|percentage/i }))
        .first();
  ...
  test("Alice: Custom fields — legal field packs loaded", async ({ page }) => {
    ...
    for (const field of fields) {
      const fieldVisible = await page
        .getByText(new RegExp(field.replace("_", "[_ ]"), "i"))
        .isVisible({ timeout: 3000 })
        .catch(() => false);
      if (!fieldVisible) {
  ...
  test("Alice: 4 matter templates listed", async ({ page }) => {
    ...
    for (const template of templates) {
      const templateVisible = await page
        .getByText(new RegExp(template, "i"))
        .first()
  ...
  test("Alice: Trust account settings page loads", async ({ page }) => {
    ...
    const createBtn = page
      .getByRole("button", { name: /create.*trust|new.*trust|add.*account/i })
      .first();
    ...
    const lpffField = page
      .locator('input[name*="lpff"], input[name*="interest"], [data-testid*="lpff"]')
      .first();
