...
test.describe.serial("Day 30 — First Billing Cycle", () => {
  ...
  test("Alice: Create fee note for Sipho Ndlovu (hourly + tariff + disbursement)", async ({
    page,
  }) => {
    await loginAs(page, "alice");
    await page.goto(`${BASE}/invoices`);
  ...
  test("Alice: Create trust FEE_TRANSFER for Moroka R8,500", async ({ page }) => {
    ...
    const transferBtn = page
      .getByRole("button", { name: /fee.*transfer|transfer|new.*transaction/i })
      .first();
  ...
  test("Alice: Fee note lifecycle — DRAFT to APPROVED to SENT", async ({ page }) => {
    ...
    const firstFeeNote = page
      .locator('table tbody tr, [data-testid*="invoice-row"], [data-testid*="fee-note-row"]')
      .first();
  ...
  test("Alice: Set fee estimate on Apex matter (R150,000)", async ({ page }) => {
    ...
    if (hasApex) {
      ...
      // Set fee estimate
      const estimateField = page
        .getByRole("textbox", { name: /budget|estimate|amount/i })
        .or(page.getByRole("spinbutton", { name: /budget|estimate|amount/i }))
        .first();
