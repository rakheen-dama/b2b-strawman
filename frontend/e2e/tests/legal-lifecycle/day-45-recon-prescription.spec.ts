...
test.describe.serial("Day 45 — Reconciliation & Prescription", () => {
  ...
  test("Alice: Start new bank reconciliation", async ({ page }) => {
    ...
    if (!mainVisible) {
      ...
      const newBtn = page
        .getByRole("button", { name: /new.*reconciliation|start.*reconciliation|reconcile/i })
        .first();
    ...
    expect
      .soft(
        await page
          .locator('form, [data-testid*="reconciliation"]')
          .first()
          .isVisible({ timeout: 5000 })
          .catch(() => false)
      )
    ...
    expect
      .soft(
        await page
          .getByText(/balanced|matched|reconciled/i)
          .isVisible({ timeout: 5000 })
          .catch(() => false)
      )
      .toBeTruthy();
  ...
  test("Alice: Check prescription tracking for Sipho (3-year personal injury)", async ({
    page,
  }) => {
    await loginAs(page, "alice");
    ...
    if (hasMatter) {
      ...
      const prescriptionTab = page
        .getByRole("tab", { name: /prescription|deadline|limitation/i })
        .first();
      ...
      const prescriptionInfo = await page
        .getByText(/prescription|3.?year|personal.?injury|limitation/i)
        .isVisible({ timeout: 5000 })
        .catch(() => false);
      if (prescriptionInfo) {
    ...
    const deadlinesVisible = await page
      .locator("main")
      .isVisible({ timeout: 10_000 })
      .catch(() => false);
    if (deadlinesVisible) {
  ...
  test("Alice: Court date lifecycle — SCHEDULED to POSTPONED, create new SCHEDULED", async ({
    page,
  }) => {
    await loginAs(page, "alice");
    ...
    if (hasDate) {
      ...
      const newDateBtn = page
        .getByRole("button", { name: /new.*date|add.*date|schedule/i })
        .first();
  ...
  test("Alice: Record payment on Apex fee note (R40,250) -> PAID", async ({ page }) => {
    ...
    if (hasApex) {
      ...
      const paymentBtn = page
        .getByRole("button", { name: /record.*payment|mark.*paid|receive.*payment/i })
        .first();
