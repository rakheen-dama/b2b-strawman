...
test.describe.serial('Day 60 — Interest Run & Investments', () => {
  ...
  test('Alice: Run interest calculation with LPFF split (6.5%)', async ({ page }) => {
    ...
    const runBtn = page.getByRole('button', { name: /run.*interest|calculate.*interest|new.*interest/i }).first()
    ...
    if (hasRun) {
      ...
      const executeBtn = page.getByRole('button', { name: /calculate|run|execute|confirm/i }).first()
  ...
  test('Alice: Place investment — Moroka R200,000 Standard Bank Fixed Deposit (s86(3))', async ({ page }) => {
    await loginAs(page, 'alice')
    await page.goto(`${BASE}/trust-accounting/investments`)
    ...
    const newBtn = page.getByRole('button', { name: /new.*investment|place.*investment|add.*investment/i }).first()
    ...
    if (hasNew) {
      ...
      const basisField = page.getByRole('combobox', { name: /basis|section.*86|authority/i }).first()
  ...
  test('Alice: Place investment — QuickCollect R30,000 Nedbank Call Account (s86(4))', async ({ page }) => {
    await loginAs(page, 'alice')
    await page.goto(`${BASE}/trust-accounting/investments`)
    ...
    if (hasNew) {
      ...
      if (hasBasis) {
        await basisField.click()
        await page.waitForTimeout(500)
        const s86Option = page.getByRole('option', { name: /client.*instruction|86\(4\)/i }).first()
        const hasS86 = await s86Option.isVisible({ timeout: 3000 }).catch(() => false)
        if (hasS86) {
          await s86Option.click()
        }
