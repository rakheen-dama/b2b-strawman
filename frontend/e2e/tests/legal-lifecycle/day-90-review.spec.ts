...
test.describe.serial("Day 90 — Quarter Review", () => {
  ...
  test("Alice: Client list shows 4+ clients", async ({ page }) => {
    ...
    const clientRows = page.locator(
      'table tbody tr, [data-testid*="client"], [data-testid*="customer"]'
    );
  ...
  test("Alice: Matters list shows multiple matters", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /Projects|Matters/i, level: 1 })).toBeVisible({
      timeout: 10_000,
    });
    await expect(page.locator("main")).toBeVisible();
    await expect(page.locator("body")).not.toContainText("Something went wrong");
    ...
    const matterRows = page.locator(
      'table tbody tr, [data-testid*="project"], [data-testid*="matter"]'
    );
  ...
  test("Alice: Fee notes list shows multiple fee notes", async ({ page }) => {
    ...
    const invoiceRows = page.locator(
      'table tbody tr, [data-testid*="invoice"], [data-testid*="fee-note"]'
    );
  ...
  test("Alice: Generate Section 35 Data Pack report", async ({ page }) => {
    ...
    const s35Btn = page
      .getByRole("button", { name: /section.*35|data.*pack|compliance.*report/i })
      .first();
  ...
  test("Alice: Profitability dashboard — revenue, costs, margins for 4 clients", async ({
    page,
  }) => {
    await loginAs(page, "alice");
    ...
    expect
      .soft(
        await page
          .getByText(/revenue|income|billing/i)
  ...
  test("Alice: Generate document for Sipho matter", async ({ page }) => {
    ...
    if (hasMatter) {
      ...
      const generateBtn = page
        .getByRole("button", { name: /generate.*document|create.*document|new.*document/i })
        .first();
      ...
      if (hasGenerate) {
        ...
        const templateOption = page
          .getByRole("option", { name: /letter|document/i })
          .or(page.locator('[data-testid*="template"]'))
          .first();
  ...
  test("Carol: Blocked from rate card settings", async ({ page }) => {
    ...
    if (mainVisible) {
      ...
      const pageText = await page
        ...
        .textContent()
        .catch(() => "");
      const isBlocked = /access.?denied|unauthorized|forbidden|not.?allowed|permission/i.test(
        pageText ?? ""
      );
