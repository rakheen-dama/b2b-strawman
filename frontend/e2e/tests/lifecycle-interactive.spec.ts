...
test.describe.serial('Interactive Lifecycle — Customer Onboarding', () => {
  ...
  test('Alice: Create a new customer via the New Customer dialog', async ({ page }) => {
    ...
    await expect(page.getByRole('dialog', { name: 'Additional Information' })).toBeVisible({ timeout: 5000 })
  ...
  test('Bob: Transition customer to ONBOARDING', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: CUSTOMER_NAME, level: 1 })).toBeVisible({ timeout: 10000 })
...
test.describe.serial('Interactive Lifecycle — Project & Time', () => {
  ...
  test('Carol: Log time on a task (existing project)', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /Annual Tax Return/, level: 1 })).toBeVisible({ timeout: 10000 })
    ...
    await expect(
      page.getByText(/Interactive test/).first()
...
test.describe.serial('Interactive Lifecycle — Invoicing', () => {
  test('Alice: View invoice list and open a draft', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /Invoices/, level: 1 })).toBeVisible({ timeout: 10000 })
  ...
  test('Alice: Approve a draft invoice', async ({ page }) => {
    ...
    const hasLinks = await table.getByRole('link').first().isVisible({ timeout: 5000 }).catch(() => false)
