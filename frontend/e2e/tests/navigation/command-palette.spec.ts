...
test.describe('CMD-01: Command Palette', () => {
  ...
  test('Cmd+K opens palette', async ({ page }) => {
    ...
    const palette = page.locator('[role="dialog"], [data-testid="command-palette"], [cmdk-dialog], [cmdk-root]').first()
    ...
    if (!paletteVisible) {
      ...
      const paletteRetry = page.locator('[role="dialog"], [data-testid="command-palette"], [cmdk-dialog], [cmdk-root]').first()
    ...
    const searchInput = palette.locator('input[type="text"], input[type="search"], input[placeholder]').first()
  ...
  test('Search navigates to page', async ({ page }) => {
    ...
    const paletteCheck = page.locator('[role="dialog"], [data-testid="command-palette"], [cmdk-dialog], [cmdk-root]').first()
  ...
  test('Escape closes palette', async ({ page }) => {
    ...
    let palette = page.locator('[role="dialog"], [data-testid="command-palette"], [cmdk-dialog], [cmdk-root]').first()
    let paletteVisible = await palette.isVisible().catch(() => false)

    if (!paletteVisible) {
      await page.keyboard.press('Control+k')
      await page.waitForTimeout(500)
      palette = page.locator('[role="dialog"], [data-testid="command-palette"], [cmdk-dialog], [cmdk-root]').first()
