...
test.describe('PORTAL-02: Portal Auth', () => {
  ...
  test('Valid token grants portal access', async ({ page }) => {
    ...
    const hasPortalContent = await page.getByText(/project/i).first().isVisible({ timeout: 5000 }).catch(() => false)
      || await page.getByText(/document/i).first().isVisible({ timeout: 3000 }).catch(() => false)
      || await page.getByText(/portal/i).first().isVisible({ timeout: 3000 }).catch(() => false)
  ...
  test('Invalid token rejected', async ({ page }) => {
    ...
    const isOnLogin = await page.getByLabel(/email/i).isVisible({ timeout: 3000 }).catch(() => false)
    const hasError = await page.getByText(/error|invalid|expired|denied|sign in/i).first().isVisible({ timeout: 3000 }).catch(() => false)
  ...
  test('No firm-side navigation leaks to portal user', async ({ page }) => {
    ...
    const hasPortalNav = await portalNav.first().isVisible({ timeout: 3000 }).catch(() => false)
