...
test.describe('PORTAL-03: Portal Navigation', () => {
  ...
  test('Portal home loads', async ({ page }) => {
    if (!portalJwt) {
      ...
      const hasEmailInput = await page.getByLabel(/email/i).isVisible({ timeout: 5000 }).catch(() => false)
      const hasPortalText = await page.getByText(/Portal/).first().isVisible({ timeout: 5000 }).catch(() => false)
    ...
    const hasPortalBranding = await page.getByText(/DocTeams Portal|Portal/).first().isVisible({ timeout: 5000 }).catch(() => false)
  ...
  test('Portal projects list', async ({ page }) => {
    ...
    const hasProjects = await page.getByText(/Projects/i).first().isVisible({ timeout: 5000 }).catch(() => false)
  ...
  test('Portal project detail', async ({ page }) => {
    ...
    const projectLink = page.getByRole('link').filter({ hasText: /Tax|Audit|Annual|Project|Bookkeeping/i }).first()
    ...
    const hasProjectName = await page.getByRole('heading', { level: 1 }).isVisible({ timeout: 10000 }).catch(() => false)
    expect(hasProjectName).toBeTruthy()

    // Should show tabs (Documents, Tasks, Comments)
    const hasDocumentsTab = await page.getByText(/Documents/).first().isVisible({ timeout: 5000 }).catch(() => false)
    const hasTasksTab = await page.getByText(/Tasks/).first().isVisible({ timeout: 5000 }).catch(() => false)
  ...
  test('Portal documents page', async ({ page }) => {
    ...
    const hasContent = await page.getByText(/document|All Shared/i).first().isVisible({ timeout: 3000 }).catch(() => false)
    ...
    const hasDocList = await page.getByText(/document|letter|All Shared/i).first().isVisible({ timeout: 3000 }).catch(() => false)
  ...
  test('Portal requests page', async ({ page }) => {
    ...
    const hasOpenTab = await page.getByText('Open').first().isVisible({ timeout: 3000 }).catch(() => false)
    const hasCompletedTab = await page.getByText('Completed').first().isVisible({ timeout: 3000 }).catch(() => false)
    const hasEmptyState = await page.getByText(/No.*requests/i).first().isVisible({ timeout: 3000 }).catch(() => false)
