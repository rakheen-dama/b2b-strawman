...
test.describe("PROP-03: Portal Proposal Acceptance", () => {
  ...
  test("Accept proposal in portal", async ({ page }) => {
    ...
    if (!canAccept) {
      test.skip(
        true,
        "Accept button not visible — proposal may not be accessible to this portal contact"
      );
      return;
    }
    ...
    const hasSuccess = await page
      ...
      .isVisible({ timeout: 5000 })
      .catch(() => false);
    const hasAcceptedBadge = await page
      .getByText("Accepted")
      .first()
      .isVisible({ timeout: 5000 })
      .catch(() => false);
  ...
  test("Decline proposal with reason in portal", async ({ page }) => {
    ...
    const contactsRes = await fetch(
      `${BACKEND_URL}/api/customers/${activeCustomer.id}/portal-contacts`,
      {
        headers: { Authorization: `Bearer ${jwt}` },
      }
    );
