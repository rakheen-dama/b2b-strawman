...
test.describe.serial('EXPENSE-01: Project Expenses', () => {
  ...
  test.beforeAll(async () => {
    ...
    const { data: project } = await apiPost('/api/projects', {
      name: PROJECT_NAME,
  ...
  test('2. Create billable expense via API and verify in UI', async ({ page }) => {
    ...
    const { status, data } = await apiPost(`/api/projects/${projectId}/expenses`, {
      date: today,
    ...
    await expect(page.getByText(`Billable expense ${RUN_ID}`).first()).toBeVisible({ timeout: 10000 })
  })
  ...
  test('3. Create non-billable expense via API', async ({ page }) => {
    ...
    await page.goto(`${BASE}/projects/${projectId}`)
    await page.getByRole('tab', { name: 'Expenses' }).click()
    await expect(page.getByText(`Non-billable expense ${RUN_ID}`).first()).toBeVisible({ timeout: 10000 })
  })
  ...
  test('4. Edit expense amount via API and verify', async ({ page }) => {
    ...
    const { status, data } = await apiPut(`/api/projects/${projectId}/expenses/${expenseId}`, {
      amount: 375.50,
      description: `Updated expense ${RUN_ID}`,
    }, token)

    expect(status).toBe(200)
    expect(Number(data.amount)).toBeCloseTo(375.50, 2)

    // Verify updated description in UI
    await loginAs(page, 'alice')
    await page.goto(`${BASE}/projects/${projectId}`)
    await page.getByRole('tab', { name: 'Expenses' }).click()
    await expect(page.getByText(`Updated expense ${RUN_ID}`).first()).toBeVisible({ timeout: 10000 })
  })
})
