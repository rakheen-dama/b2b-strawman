...
test.describe.serial('PROJ-03: Time Entries', () => {
  ...
  test.beforeAll(async () => {
    ...
    // Create task
    const { data: task } = await apiPost(`/api/projects/${projectId}/tasks`, {
      title: TASK_TITLE,
    }, token)
    taskId = task.id
  })
  ...
  test('2. Edit time entry via API and verify', async ({ page }) => {
    ...
    const { status, data } = await apiPut(`/api/time-entries/${timeEntryId}`, {
      durationMinutes: 180,
    ...
    await expect(
      page.getByText(/Edited time/).first()
  ...
  test('3. Delete time entry via API and verify', async ({ page }) => {
    ...
    const entries = await apiGet(`/api/tasks/${taskId}/time-entries`, token) as Array<{ id: string }>
  ...
  test('4. Time entry captures rate snapshot', async ({ page }) => {
    ...
    const { status, data } = await apiPost(`/api/tasks/${taskId}/time-entries`, {
  ...
  test('6. Mark time entry non-billable', async ({ page }) => {
    ...
    const nonBillable = entries.find((e) => e.description?.includes(`Non-billable entry ${RUN_ID}`))
