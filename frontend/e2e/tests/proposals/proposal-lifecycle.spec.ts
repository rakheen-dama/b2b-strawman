...
async function createProposalViaApi(jwt: string, title: string): Promise<string> {
  ...
  const activeCustomer = customerList.find(
    (c: { lifecycleStatus: string }) => c.lifecycleStatus === 'ACTIVE' || c.lifecycleStatus === 'ONBOARDING'
  )
...
test.describe('PROP-02: Proposal Lifecycle', () => {
  ...
  test('DRAFT to SENT via UI', async ({ page }) => {
    ...
    if (dialogVisible) {
      ...
      const noContacts = await page.getByText(/No portal contacts/i).isVisible({ timeout: 3000 }).catch(() => false)
      if (noContacts) {
      ...
      const loadingContacts = await page.getByText(/Loading contacts/i).isVisible({ timeout: 1000 }).catch(() => false)
      if (loadingContacts) {
        await page.waitForTimeout(3000)
      }

      // Check again for no contacts after loading
      const stillNoContacts = await page.getByText(/No portal contacts/i).isVisible({ timeout: 2000 }).catch(() => false)
      if (stillNoContacts) {
        test.skip(true, 'No portal contacts configured for this customer')
        return
      }
      ...
      const recipientTrigger = page.locator('#send-recipient')
    ...
    await page.reload()
    const hasSent = await page.getByText('Sent').first().isVisible({ timeout: 10000 }).catch(() => false)
    const hasDraft = await page.getByText('Draft').first().isVisible({ timeout: 3000 }).catch(() => false)
    if (!hasSent && hasDraft) {
  ...
  test('SENT to ACCEPTED via API', async () => {
    ...
    const contactsRes = await fetch(`${BACKEND_URL}/api/customers/${proposal.customerId}/portal-contacts`, {
      headers: { Authorization: `Bearer ${jwt}` },
    })
