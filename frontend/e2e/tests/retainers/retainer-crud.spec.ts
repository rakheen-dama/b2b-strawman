...
test.describe("RET-01: Retainer CRUD", () => {
  test("View retainer list", async ({ page }) => {
    ...
    await expect(page.locator("h1").filter({ hasText: "Retainers" })).toBeVisible({
      timeout: 10000,
    });
    ...
    const hasRetainers = await page
      .getByRole("link")
      .first()
      .isVisible({ timeout: 5000 })
      .catch(() => false);
    const hasEmptyState = await page
      .getByText(/No retainers/i)
  ...
  test("View retainer detail", async ({ page }) => {
    ...
    const retainerLink = page
      .getByRole("link")
      .filter({ hasText: /Retainer|Monthly|Hour|Bookkeeping/i })
      .first();
    ...
    const hasDetails = await page
      .getByText(/Hour Bank|Fixed Fee|Monthly|Quarterly/i)
      .first()
      .isVisible({ timeout: 5000 })
      .catch(() => false);
  ...
  test("Create retainer", async ({ page }) => {
    ...
    const hasNewRetainer = await page
      .getByText(retainerName)
      .first()
      .isVisible({ timeout: 5000 })
      .catch(() => false);
