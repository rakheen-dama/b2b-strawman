...
test.describe('DP-02: Data Protection Settings', () => {
  ...
  test('Set jurisdiction to South Africa', async ({ page }) => {
    ...
    if (bodyText?.includes('Something went wrong') || bodyText?.includes('404')) {
      await page.goto(`${base}/settings`)
      const dpTab = page.locator('[role="tab"]:has-text("Data Protection"), a:has-text("Data Protection")').first()
      if (await dpTab.isVisible().catch(() => false)) {
        await dpTab.click()
        await page.waitForTimeout(1000)
      } else {
    ...
    const jurisdictionSelect = page.locator('select[name*="jurisdiction"], [data-testid="jurisdiction-select"]').first()
  ...
  test('Set information officer', async ({ page }) => {
    ...
    const nameInput = page.locator('input[name*="officer" i][name*="name" i], input[placeholder*="officer" i]').first()
  ...
  test('View retention policies', async ({ page }) => {
    ...
    const retentionTab = page.locator('[role="tab"]:has-text("Retention"), a:has-text("Retention")').first()
  ...
  test('Edit retention period', async ({ page }) => {
    ...
    const periodInput = page.locator('input[type="number"][name*="retention" i], input[name*="months" i]').first()
  ...
  test('View processing register', async ({ page }) => {
    ...
    const registerTab = page.locator('[role="tab"]:has-text("Processing"), a:has-text("Processing"), a:has-text("Register")').first()
  ...
  test('View DSAR list', async ({ page }) => {
    ...
    const dsarTab = page.locator('[role="tab"]:has-text("DSAR"), a:has-text("DSAR"), [role="tab"]:has-text("Requests")').first()
