...
export async function getFieldDefinitions(entityType: string): Promise<FieldDefinitionResponse[]> {
  return api.get<FieldDefinitionResponse[]>(`/api/field-definitions?entityType=${entityType}`);
}

export async function getFieldDefinition(id: string): Promise<FieldDefinitionResponse> {
...
export async function getFieldGroups(entityType: string): Promise<FieldGroupResponse[]> {
  return api.get<FieldGroupResponse[]>(`/api/field-groups?entityType=${entityType}`);
}

export async function getFieldGroup(id: string): Promise<FieldGroupResponse> {
  return api.get<FieldGroupResponse>(`/api/field-groups/${id}`);
}

export async function createFieldGroup(req: CreateFieldGroupRequest): Promise<FieldGroupResponse> {
...
export async function getGroupMembers(groupId: string): Promise<FieldGroupMemberResponse[]> {
  return api.get<FieldGroupMemberResponse[]>(`/api/field-groups/${groupId}/fields`);
}
...
export async function setEntityFieldGroups(
  ...
  return api.put<FieldDefinitionResponse[]>(`/api/${prefix}/${entityId}/field-groups`, {
    appliedFieldGroups,
  });
}
