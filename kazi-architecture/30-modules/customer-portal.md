...
| `GET /portal/deadlines`, `/{sourceEntity}/{id}` | `PortalDeadlineController` `→ customerbackend/controller/PortalDeadlineController.java` | Module `deadlines` (`PortalDeadlineService.MODULE_ID` — the customer-portal deadline view, distinct from the firm-side `regulatory_deadlines` module); profiles `accounting-za, legal-za`. ADR-256 polymorphic-portal-deadline-view. |
