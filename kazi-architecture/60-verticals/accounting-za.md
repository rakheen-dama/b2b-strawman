...
### 9.5 `regulatory_deadlines` was missing from `enabledModules` — RESOLVED

**Resolution:** the earlier "slug drift" framing was wrong. `deadlines` and `regulatory_deadlines` are **two distinct, both-valid modules**, not the same slug spelled two ways:

- `regulatory_deadlines` — the firm-side SARS/CIPC deadline calendar. Registered in `VerticalModuleRegistry` (`→ VerticalModuleRegistry.java:86`) with `defaultEnabledFor: ["accounting-za"]`, and guarded by `DeadlineController` (5 endpoints, `requireModule("regulatory_deadlines")`) and `ProjectUpcomingDeadlinesService`.
- `deadlines` — the **customer-portal** deadline view (Epic 497A, ADR-254/256), owned by `PortalDeadlineService.MODULE_ID` (`→ customerbackend/service/PortalDeadlineService.java`). Intentionally enabled in both accounting-za and legal-za.

The actual defect was that `accounting-za.json` listed only `deadlines` and **omitted** `regulatory_deadlines`, so the firm-side `DeadlineController` fail-closed for accounting tenants despite the registry declaring the module default-enabled for them. **Fixed:** `regulatory_deadlines` was added to `enabledModules` (kept `deadlines`). The reconciliation seeder (`VerticalProfileReconciliationSeeder`, add-only merge) back-propagates the new module to existing tenants on the next app start, so no manual backfill is needed. `VerticalProfileModuleSlugValidationTest` now guards against both regression directions (unknown slug; registry default not honoured by the profile JSON).
