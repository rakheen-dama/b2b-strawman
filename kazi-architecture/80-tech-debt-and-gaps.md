...
2. **R-02 Observability is thin** — High, L. A Prometheus exporter and two custom meter sets exist (job-queue counters/timers, per-shard HikariCP pool gauges), and prod exposes `/actuator/prometheus`. But there is **no per-tenant metric tagging**, no scheduled-job (`@Scheduled` poller) success/failure metric, no integration-adapter latency, and no dashboards or alerting on top. First sign of production trouble is still likely a user report. Source: `20-cross-cutting/observability.md`.
...
| A-02 | Observability is thin | High | L | Prometheus exporter present (`micrometer-registry-prometheus` in `backend/pom.xml`; prod exposes `/actuator/prometheus`). Custom meters exist for the job queue and per-shard pools, but no `@Timed`/`@Observed` annotations, no per-tenant metric tag, no `@Scheduled`-poller success/failure metric, no OpenTelemetry tracing, no dashboards/alerting. | `observability.md` |
...
| D-02 | Frontend and portal duplicate Shadcn + terminology + utilities | Med | L | No shared package between `frontend/` and `portal/`. Each maintains its own `components/ui/`, its own `terminology-map.ts`, its own format helpers. Drift is observable; refactor candidate. **Mostly resolved (Wave 2.4 + 2.5).** 2.4 extracted Shadcn primitives + the `cn` class-combiner into `@b2mash/ui` (`packages/ui`). 2.5 extracted the vertical-terminology system into `@b2mash/shared` (`packages/shared`): one BASE map (the staff-app superset) plus an explicit, commented `PORTAL_TERMINOLOGY_OVERRIDES` layer (e.g. `Matters → Engagements` for accounting-za, which the portal sidebar needs and the staff app must not have) — silent drift replaced by an explicit override mechanism. Both apps' `lib/utils.ts` now thin-re-export `cn` from `@b2mash/ui`; terminology imports rewritten to `@b2mash/shared/terminology-map`. Reconciliation verified behaviour-preserving (old vs new map, both views, all renders identical except the documented intentional override). **Fully resolved (Wave 2.6, user-approved 2026-06-11).** `format.ts` is now unified in `@b2mash/shared/format` on the **portal convention** (en-GB day-first dates + en-ZA currency style across both apps; client-facing portal output unchanged, staff app changed to match — explicit product decision). The shared module takes the SUPERSET of helpers from both files (portal's robust `formatCurrency`/`formatFileSize`/`formatRelativeDate` semantics plus the staff app's extra helpers — `formatCurrencySafe`, `formatDateShort`, `formatLocalDate`, `formatProposalExpiresAt`, `formatComplianceDate(WithTime)`, `formatDuration`, `computeDuration`, `isOverdue` — rendered in the unified style). The portal deletes its local `format.ts` and imports `@b2mash/shared/format` directly (20 import-line rewrites, zero test changes — client-facing output proven byte-identical via an old-vs-new harness). The staff app keeps a thin `frontend/lib/format.ts` re-export shim whose ONLY job is to load `@/lib/intl-polyfill` as a side-effect — the `@formatjs` NumberFormat polyfill is required for correct en-ZA SSR rendering on Node small-icu (regression GAP-L-12) and is staff-app-specific (the portal runs on full-ICU runtimes and must NOT load it, or its byte-output would change), so the polyfill stays out of the shared package. Intentional staff-app rendering changes (all test expectations updated, reviewable): USD `"$18,750.00"` → `"US$18 750,00"`; EUR `"18.750,00 €"` → `"€18 750,00"`; dates `"May 1, 2026"` → `"1 May 2026"`; file sizes now always 1-decimal + a 5th unit (TB) — fixes a latent `"5.0 undefined"` bug for terabyte inputs. | `_discovery/A6-cross-cutting.md`; `70-repos/portal.md` |
...
| H-04 | `automation-legal-za` pack not referenced in `legal-za.json` | — | S | `verticals/legal-za.json` | **Invalid — premise disproven.** Claim was that the missing `packs.automation` key in `legal-za.json` prevents auto-install. But provisioning installs automation packs via `TenantProvisioningService.installPacksViaPipeline(…, AUTOMATION_TEMPLATE)` → `PackCatalogService.getPackIdsForProfile`, which filters by each pack's own `verticalProfile` field (`AutomationPackInstaller.availablePacks()` → `pack.verticalProfile()`), **not** by the profile JSON's `packs.automation` key. `automation-templates/legal-za.json` declares `verticalProfile: legal-za`, so legal-za tenants already auto-install all 5 rules at provisioning — identical to consulting-za (which `ConsultingZaAutomationPackTest` proves). The `packs.*` map in the profile JSON is vestigial: it's read only by two tests asserting `containsKey("field")`, never by the install pipeline (accounting-za's and consulting-za's `packs.automation` keys are equally inert). Verified by `LegalZaAutomationPackTest` (4 green tests on unchanged production code). No code/JSON change needed. (Duplicate B-05 removed.) |
...
| ID | Gap | Sev | Effort | Detail | Source |
|---|---|---|---|---|---|
| I-01 | Metric coverage is thin (exporter present) | High | M | Prometheus/Micrometer registry **does** exist in the backend; gap is breadth + no dashboards/alerting. See A-02. | `observability.md` |
...
| J-05 | Portal duplicates Shadcn install | Low | M | See D-02. Each app has its own `components/ui/`, `components.json`. **Resolved (Wave 2.4):** shared Shadcn primitives + `cn` extracted to `@b2mash/ui` (`packages/ui`); both apps consume it via subpath imports. | `70-repos/portal.md` |
...
3. Remove `RetainerPeriod.PeriodStatus.INVOICED` if confirmed dead (E-03, H-09).
4. Add `verticalModuleGuard.requireModule("retainer_agreements")` to retainer service entry points (B-10).
5. Add a boot-time warning when `VerticalProfileRegistry` skips a malformed JSON (G-01, K-05).
6. Rename `ProjectStatus.VALID_TRANSITIONS` ↔ `ALLOWED_TRANSITIONS` to a single name (E-04).
7. Mark superseded ADRs (139, 156, 010, 013, 014, 219, 222) with explicit `Status: Superseded by ADR-XXX` headers in the ADR files (B-08).
