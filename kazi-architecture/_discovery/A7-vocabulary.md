# A7 — Kazi Ubiquitous Language Glossary

**Generated:** 2026-05-10
**Scope:** all 4 repos — `backend/`, `frontend/`, `gateway/`, `portal/`.
**Method:** entity catalogue (A1) + frontend types (A2) + portal types (A3) + enum extraction + REST path nouns + terminology-map cross-check.
**Convention:** human-readable term form. One canonical code anchor per row. Rows that exist in product talk only are flagged as `gap`.

Read this in 10 minutes and you can speak Kazi reliably. The Watch-words section at the bottom is the quick translator from "what someone said" to "what we actually call it".
...
|---|---|---|---|---|
| Acceptance Request | backend entity, frontend type, REST path, ADR (acceptance flow) | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/acceptance/AcceptanceRequest.java:22` | proposals-acceptance | Token-gated lightweight e-sign flow attaching a `GeneratedDocument` to a `PortalContact`. Watch: not the same as "Acceptance Test" (QA). Status enum: `PENDING, SENT, VIEWED, ACCEPTED, EXPIRED, REVOKED`. |
| Access Request | backend entity, REST path `/api/access-requests`, REST path `/api/platform-admin/access-requests` | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/accessrequest/AccessRequest.java:18` | platform-administration | Public OTP-verified sign-up application; reviewed by Platform Admin. Synonyms to retire: "Sign-up", "Registration", "Self-Service Onboarding" (Kazi has none — admin-gated only). |
| Access Request Status | Java enum | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/accessrequest/AccessRequestStatus.java:3` | platform-administration | Values: `PENDING_OTP, OTP_VERIFIED, APPROVED, REJECTED` (typical). |
...
| Action Item | terminology override (legal-za only) | `frontend/lib/terminology-map.ts:49` | terminology, legal-vertical | Legal-vertical UI label for a Task. Always backed by `Task` entity. |
| Action Type | Java enum | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/automation/ActionType.java:3` | automation-engine | Values: `CREATE_TASK, SEND_NOTIFICATION, SEND_EMAIL, UPDATE_STATUS, CREATE_PROJECT, ASSIGN_MEMBER, INVOKE_AI_SPECIALIST`. |
| Activity Feed | REST path `/api/dashboard/activity`, portal route `/activity` | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/portal/PortalActivityService.java:28` | notifications-activity, portal | Cross-entity event timeline. Portal activity has tabs: `MINE, FIRM`. Watch: not the Audit Log (admin-only, immutable). |
| Adverse Party | backend entity, frontend type, REST path `/api/legal/adverse-parties`, portal? no | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/verticals/legal/conflictcheck/AdverseParty.java` | legal-vertical | Legal-only. Used in conflict-check workflow. Has `AdversePartyLink` join to matters. |
| Adverse Party Link | backend entity, frontend type | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/verticals/legal/conflictcheck/AdversePartyLink.java` | legal-vertical | Join table linking `AdverseParty` ↔ `Project` (Matter). |
...
| AI Invocation | frontend module, REST path, AI Queue page | `frontend/lib/api/ai-invocations.ts` | automation-engine, ai-assistant | A call into the AI specialist registry, queued for review. Backend `InvocationStatus` enum. |
| AI Queue | frontend route `/settings/automations/ai-queue` | `frontend/app/(app)/org/[slug]/settings/automations/ai-queue/page.tsx` | automation-engine, ai-assistant | Review queue for AI specialist invocations triggered by automation `INVOKE_AI_SPECIALIST` action. |
...
| Allocation | backend entity `ResourceAllocation`, REST path `/api/allocations` | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/capacity/ResourceAllocation.java:16` | capacity-resource-planning | Member×project×week hours commitment. Watch: distinct from Capacity (the available hours). |
...
| Audit Event | backend entity, REST path `/api/audit-events`, ADR-029, ADR-260 | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/audit/AuditEvent.java:29` | notifications-activity | Append-only, DB-trigger-immutable. Synonyms to retire: "log entry", "activity record" (those refer to Activity Feed). |
...
| Audit Trail | terminology override label (legal-za) | `frontend/lib/terminology-map.ts:91` | terminology, legal-vertical | Legal-UI label for Audit Log. Same data. |
...
| Bank Statement | frontend type, trust reconciliation | `frontend/lib/types/trust.ts` (BankStatementResponse) | legal-vertical | Imported file (CSV/OFX) for trust reconciliation matching. |
...
| Billable | field on `TimeEntry`, `Expense`; flag | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/timeentry/TimeEntry.java:17` | tasks-time, invoicing-billing | Boolean meaning "should appear on an invoice". Distinct from Billed (already invoiced) and Billing Status enum. |
| Billing Method | Java enum | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/billing/BillingMethod.java:7` | platform-administration | Platform-level subscription billing. Values: `PAYFAST, DEBIT_ORDER, PILOT, COMPLIMENTARY, MANUAL`. Watch: this is HOW the org pays Kazi — not how the org bills its customers. |
| Billing Rate | frontend type, REST path `/api/settings/rates` | `frontend/lib/types/billing.ts` (BillingRate) | invoicing-billing | Price per hour charged to customer. Distinct from Cost Rate (internal cost). Aliases per profile: "Tariff Schedule" (legal), "Fee Schedule" (accounting), "Billing Rates" (consulting). Watch: legacy product term "Rate Card" — retire in favour of "Billing Rate" (the canonical type). |
| Billing Run | backend entity, REST path `/api/billing-runs`, frontend page | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/billingrun/BillingRun.java:18` | invoicing-billing | Bulk invoice generation across many customers. Module-gated `bulk_billing`. Watch: not a Billing Cycle — Kazi has no subscription cycles for end-customer billing. |
...
| Billing Run Status | Java enum | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/billingrun/BillingRunStatus.java` | invoicing-billing | Run lifecycle: typically `DRAFT, GENERATING, READY, APPROVED, SENDING, SENT, CANCELLED`. |
| Billing Status (TimeEntry) | Java enum | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/timeentry/BillingStatus.java:13` | tasks-time, invoicing-billing | `UNBILLED, BILLED, NON_BILLABLE`. Watch: there are three "BillingStatus" enums in the codebase — TimeEntry, Expense (`ExpenseBillingStatus`), Disbursement (`DisbursementBillingStatus`). Always disambiguate. |
...
| Branding | REST path `/portal/branding`, `/api/settings`, frontend setting | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/portal/PortalBrandingController.java` | settings, portal | Customer-facing logo + colour + name. Surfaced to portal pre-auth on login screen. |
...
| Budget Status | Java enum `BudgetStatusEnum` | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/budget/BudgetStatus.java:28` | invoicing-billing | Watching: `OK, NEAR_LIMIT, OVER`. |
| Capability | Java enum, REST path `/api/me/capabilities` | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/orgrole/Capability.java:7` | identity-access | Atomic permission. Values include: `FINANCIAL_VISIBILITY, INVOICING, PROJECT_MANAGEMENT, TEAM_OVERSIGHT, CUSTOMER_MANAGEMENT, AUTOMATIONS, RESOURCE_PLANNING, MANAGE_COMPLIANCE, MANAGE_COMPLIANCE_DESTRUCTIVE, VIEW_LEGAL, MANAGE_LEGAL, VIEW_TRUST, MANAGE_TRUST, APPROVE_TRUST_PAYMENT, MANAGE_DISBURSEMENTS, APPROVE_DISBURSEMENTS, WRITE_OFF_DISBURSEMENTS, CLOSE_MATTER, OVERRIDE_MATTER_CLOSURE, GENERATE_STATEMENT_OF_ACCOUNT`. Watch: not "Permission", not "Role" (Role is a bundle). |
...
| Clause | backend entity `DocumentClause`, frontend page `/settings/clauses` | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/clause/` (in `template/` package) | documents-templates | Reusable doc fragment included in a `DocumentTemplate`. |
...
| Clerk User Id | field `Member.clerkUserId` | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/member/Member.java:22` | identity-access | Legacy field name from Clerk era — Clerk is fully removed (now Keycloak). Field still named `clerkUserId` but represents any external user id (Keycloak `sub`). Watch: code says Clerk; production uses Keycloak. |
| Client | terminology override (all verticals) | `frontend/lib/terminology-map.ts:3` | terminology | UI label for `Customer` in legal-za, accounting-za, consulting-za. Watch: code always says Customer. Never rename the entity. |
| Client Ledger | frontend type, REST path `/portal/trust/...`, route `/trust-accounting/client-ledgers` | `frontend/lib/types/trust.ts` (ClientLedgerCard) | legal-vertical | Per-client trust money ledger. Statutory legal-vertical concept. |
...
| Completeness Score | frontend type, backend service | `frontend/lib/types/common.ts` (CompletenessScore) | customer-lifecycle | % of required custom fields filled. Drives prerequisite gates. |
| Concrete Studio | design-system palette name | `frontend/CLAUDE.md` | (frontend design tokens) | Light-mode token palette. Watch: not user-facing language. |
...
| Conflict Check | backend entity, frontend type, REST path `/api/legal/conflict-checks`, route | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/verticals/legal/conflictcheck/ConflictCheck.java` | legal-vertical | Conflict-of-interest scan for new matter intake. |
...
| Cost Rate | frontend type | `frontend/lib/types/billing.ts` (CostRate) | invoicing-billing | Internal cost per member-hour. Used for profitability. Watch: distinct from Billing Rate. Never visible to customer. |
...
| Customer | backend entity, frontend type, REST path `/api/customers` | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/customer/Customer.java:23` | customer-lifecycle | The end-client of the tenant org. Watch: vertical UI labels: "Client" (all 3 profiles). Retire: "Account" (CRM term — never use), "Lead" (no such status — `PROSPECT` is the lifecycle stage), "Contact" (means PortalContact). |
...
| Customer Type | Java enum | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/customer/CustomerType.java:4` | customer-lifecycle | `INDIVIDUAL, COMPANY, TRUST`. Watch: TRUST here means a legal/tax trust entity — NOT trust accounting. |
...
| Deadline | frontend type, REST path `/api/legal/deadlines`, `/portal/deadlines`, page | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/...` (deadline service) | legal-vertical | Module `regulatory_deadlines`. Watch: distinct from Project Due Date and Court Date. |
...
| Disbursement | backend entity (legal), invoice line type, terminology override | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/verticals/legal/disbursement/` | legal-vertical, invoicing-billing | Out-of-pocket expense passed through to client. Legal-vertical UI: replaces "Expense" entirely. Invoice line type `DISBURSEMENT`. |
...
| Disbursement Billing Status | Java enum | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/verticals/legal/disbursement/DisbursementBillingStatus.java:9` | legal-vertical, invoicing-billing | Whether disbursement has been billed. |
...
| Document | backend entity, frontend type, REST path | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/document/Document.java:16` | documents-templates | Uploaded file with `scope` (PROJECT/CUSTOMER) and `visibility`. Watch: distinct from `GeneratedDocument`. |
...
| Expense | backend entity, frontend type, REST path | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/expense/Expense.java:19` | tasks-time, invoicing-billing | Reimbursable cost. Legal-UI label: "Disbursement". Watch: `ExpenseCategory` enum is generic; legal `DisbursementCategory` is the legal-vertical equivalent. |
...
| Expense Category | Java enum | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/expense/ExpenseCategory.java:4` | tasks-time | Generic categories. Watch: legal vertical uses `DisbursementCategory` instead. |
...
| Fee Schedule | terminology override (accounting-za) | `frontend/lib/terminology-map.ts:33` | terminology, accounting-vertical | UI label for `Rate Card` / Billing Rate. |
...
| Information Request | backend entity, REST path `/api/information-requests`, portal page | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/informationrequest/InformationRequest.java:22` | information-requests | Structured client data-collection workflow. Watch: not the same as Data Subject Request. |
| Information Request Status | Java enum `RequestStatus` | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/informationrequest/RequestStatus.java:4` | information-requests | Lifecycle: typically `DRAFT, SENT, IN_PROGRESS, COMPLETED, CANCELLED`. |
...
| Invitation | backend entity `PendingInvitation`, REST path | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/invitation/PendingInvitation.java:22` | identity-access | App-managed role invite to join the org. Watch: distinct from Keycloak invitations. |
...
| Invoice | backend entity, frontend type, REST path | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/invoice/Invoice.java:24` | invoicing-billing | Bill to a Customer. Legal-UI label: "Fee Note". |
...
| KYC | integration domain, frontend type | `frontend/lib/types/kyc.ts` | integrations-pack-system, customer-lifecycle | Identity verification adapter. Watch: in legal-za context implies FICA. |
...
| Lifecycle Action | Java enum | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/compliance/LifecycleAction.java:3` | customer-lifecycle | The discrete user/system actions that move customers between lifecycle statuses (ACTIVATE, OFFBOARD, ANONYMIZE, etc.). |
| Lifecycle Status | Java enum, frontend type | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/customer/LifecycleStatus.java:7` | customer-lifecycle | Values: `PROSPECT, ONBOARDING, ACTIVE, DORMANT, OFFBOARDING, OFFBOARDED, ANONYMIZED`. The state machine is enforced. Watch: `PROSPECT` is the default for new customers (not "Lead"). |
...
| Magic Link | concept, REST path `/portal/auth/request-link`, entity `MagicLinkToken` | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/portal/MagicLinkToken.java:19` | portal | One-time email-delivered token for portal sign-in. Watch: also used for token-gated `Acceptance Request` and `Proposal` flows. |
...
| Matter | terminology override (legal-za) | `frontend/lib/terminology-map.ts:45` | terminology, legal-vertical | UI label for `Project` in legal vertical. Always backed by `Project` entity. Backend never uses "Matter" except in the `MatterClosure*` legal-specific service classes. |
...
| Member | backend entity, REST path `/api/members` | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/member/Member.java:22` | identity-access | A user inside a tenant org. Linked by `clerkUserId` to the IDP. Watch: distinct from `PortalContact` (external customer-side user) and `ProjectMember` (project access grant). |
...
| Onboarding | lifecycle status | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/customer/LifecycleStatus.java:7` | customer-lifecycle | The active intake phase before `ACTIVE`. Watch: not the platform sign-up (that's Access Request). |
| Org | abbreviation for Organization (URL slug `/org/[slug]`) | `frontend/app/(app)/org/[slug]/layout.tsx` | tenancy-provisioning | URL/UI shorthand. Watch-words: "Tenant" (used backend-side for schema), "Workspace" (never used in Kazi — retire if heard). |
...
| Org Role | backend entity, REST path `/api/org-roles` | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/orgrole/OrgRole.java:23` | identity-access | A bundle of capabilities. System and custom roles. Watch: not a Project Role. |
...
| Organization | backend entity (public schema) | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/provisioning/Organization.java` | tenancy-provisioning | The tenant. Watch: short form is "Org". Avoid "Company" (legacy term — `Customer.customerType=COMPANY` is unrelated). |
...
| Pack | backend, REST path `/api/packs` | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/packs/` | integrations-pack-system | Bundle of seedable content. Five pack types. Watch: not a software dependency package. |
...
| Payment Event | backend, REST path `/api/invoices/payment-events` | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/invoice/PaymentEventStatus.java:4` | invoicing-billing | Recorded receipt against an invoice (inc. partial reversals). |
...
| Payment Status (integration) | Java enum | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/integration/payment/PaymentStatus.java:7` | integrations-pack-system | Adapter-level payment outcome. Watch: third "PaymentStatus"-named enum — also exists for SubscriptionPayment. |
...
| Platform Admin | role/group, REST path prefix `/api/platform-admin/*`, filter | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/security/PlatformAdminFilter.java` | platform-administration | Cross-tenant Kazi staff. Identified by JWT `groups` claim. Watch: distinct from Org Owner role (per-tenant). |
| Portal | app `portal/`, package `backend/.../portal` | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/portal/` | portal | Customer-facing Next.js app. Watch: do not call it "Client Portal" in code (UI label OK). |
...
| Portal Contact | backend entity | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/portal/PortalContact.java:16` | portal | A person on the customer side who can use the portal. Has roles `PRIMARY, BILLING, GENERAL` and statuses `ACTIVE, SUSPENDED, ARCHIVED`. Watch: divergence — frontend type for this is sometimes called "Customer Contact" in UI; backend code always says PortalContact. |
...
| Portal JWT | concept, `localStorage` key `portal_jwt` | `portal/lib/auth.ts` | portal, identity-access | Custom JWT, NOT an OAuth2 token. Stored client-side. Watch: completely separate from staff-frontend Keycloak session. |
...
| Prerequisite | service, REST path `/api/customers/{id}/setup-status` etc. | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/prerequisite/PrerequisiteContext.java:4` | customer-lifecycle | Required-fields gate before lifecycle transitions or document generation. |
...
| Project | backend entity, frontend type, REST path | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/project/Project.java:24` | projects | Core deliverable unit. UI labels: "Matter" (legal), "Engagement" (accounting). Watch: Backend never says Matter except in legal closure subpackage. |
...
| Project Member | backend entity | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/member/ProjectMember.java:14` | projects, identity-access | Per-project access grant. Has `projectRole`. Watch: distinct from Org Member — a Member can be on the org but not a project. |
...
| Project Role | frontend type, field `ProjectMember.projectRole` | `frontend/lib/types/member.ts:3` | projects | Project-scoped role (lead/contributor/observer). Watch: distinct from Org Role. |
| Project Status | Java enum, frontend type | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/project/ProjectStatus.java:7` | projects | `ACTIVE, COMPLETED, ARCHIVED, CLOSED`. `CLOSED` is legal-vertical only (compliance-gated, retention anchor). Watch: `CLOSED` is NOT terminal — supports `reopen`. |
...
| Retainer Frequency | Java enum | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/retainer/RetainerFrequency.java:5` | invoicing-billing | Period cadence: `MONTHLY, QUARTERLY, ANNUALLY`. |
...
| Rolllover Policy | Java enum | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/retainer/RolloverPolicy.java:3` | invoicing-billing | Whether unused retainer hours roll into the next period. |
...
| Schedule (Project) | backend entity (`ProjectSchedule`), REST path `/api/schedules` | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/projecttemplate/` | projects | Recurrence config for instantiating projects from a `ProjectTemplate`. Watch: not a Calendar Event. |
...
| Subscription | backend entity (public schema) | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/billing/Subscription.java:19` | platform-administration | Org's Kazi platform subscription. Watch: NOT a customer-facing concept — Kazi has no plan tiers per `feedback_no_plan_subscriptions`. |
...
| Tariff Schedule | backend entity, frontend type, terminology override | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/verticals/legal/tariff/TariffSchedule.java:19` | legal-vertical | LSSA tariff. UI label for `Rate Card` in legal-za. Watch: this term is overloaded — the entity is legal-vertical, the terminology label means "Billing Rates". |
| Task | backend entity, frontend type, REST path | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/task/Task.java:26` | tasks-time | Unit of work inside a Project. UI label: "Action Item" (legal-za). |
...
| Template Source | Java enum | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/template/TemplateSource.java:3` | documents-templates | `SYSTEM, PACK, CUSTOM`. There is also an `informationrequest` `TemplateSource` enum — disambiguate. |
| Tenant | concept (backend `multitenancy`), `TENANT_ID` ScopedValue | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/multitenancy/RequestScopes.java` | tenancy-provisioning | Backend-internal name for the schema/Org. Watch: end-users say "Org"; backend says "Tenant". Same concept. |
...
| Tiptap | rendering technology | `architecture/ARCHITECTURE.md` | documents-templates | Rich-text JSON format used in `DocumentTemplate.content`. Watch: feedback rule — never call this Thymeleaf. |
...
| Trigger Type | Java enum | `backend/src/main/java/io/b2mash/b2b/b2bstrawman/automation/TriggerType.java:3` | automation-engine | Values: `TASK_STATUS_CHANGED, PROJECT_STATUS_CHANGED, CUSTOMER_STATUS_CHANGED, INVOICE_STATUS_CHANGED, TIME_ENTRY_CREATED, BUDGET_THRESHOLD_REACHED, DOCUMENT_ACCEPTED, INFORMATION_REQUEST_COMPLETED, PROPOSAL_SENT, FIELD_DATE_APPROACHING, SCHEDULED`. |
...
| Workflow | (gap) — common product term | (none) | (n/a) | No `Workflow` entity. Use "Automation Rule" instead. Retire "workflow" in conversation when an automation rule is meant. |
...
Quick reference. Heard this? Use this instead.
...
| Engagement | Project (entity) — UI label "Engagement" only in accounting-za | Use `Project` in code/conversation; only render "Engagement" in accounting UI. |
...
| Rate Card | Billing Rate (entity) — UI label "Tariff Schedule" / "Fee Schedule" / "Billing Rates" per profile | "Rate Card" is a frontend label; the type is `BillingRate`. |
...
## Known divergences flagged in this glossary
