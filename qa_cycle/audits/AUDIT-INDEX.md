...
| [02-flyway-default-drift.md](./02-flyway-default-drift.md) | `ADD COLUMN ... DEFAULT X` only applies to new INSERTs; pre-existing rows have NULL/empty. | 0 confirmed; **5 suspect** rows. | 5 | **Downgraded 2026-05-01** — user confirmed no production data, backward data compat not a priority. Re-provision instead of backfill. Forward-looking convention still worth documenting. |
