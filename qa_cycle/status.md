...
| OBS-801 | Portal acceptance banner uses "project" instead of "matter" (legal-za terminology leak) | LOW | Dev | SPEC_READY | 8 | Hardcoded message in `PortalProposalService.java:148`. Portal pages display `result.message` without applying `t()`. Fix: compose message on portal side using `t("project")` / `t("proposal")`. Fix spec: `qa_cycle/fix-specs/OBS-801.md`. |
