...
- **Next checkpoint**: Day 3.1 — Create RAF matter (Bob continues)
...
| OBS-201 | Create Client dialog has TWO ID-related fields (Step 1 generic "ID Number" + Step 2 legal-promoted "ID / Passport Number"); Step 2 SA Legal group is buried under two stacked accordions. | nit | Product | OPEN | 2 | Suggest: drop the generic Step-1 "ID Number" when vertical=`legal-za` (or rename); auto-expand the SA Legal field group; UX nit, not a blocker. |
| OBS-202 | No "Run KYC Verification" button on client detail. KYC adapter not wired in this build. | exempt | Product | OPEN-EXEMPT | 2 | User mandate explicitly permits KYC as a legitimate open gap. Will remain OPEN-EXEMPT for the duration of the cycle. |
