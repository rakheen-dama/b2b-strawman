...
|----|---------|----------|--------|-------|----|-------|
| GAP-PE-001 | Magic link email subject says "DocTeams" instead of firm name | LOW | SPEC_READY | -- | -- | **Bug.** `EmailContextBuilder.resolveOrgName()` reads `RequestScopes.getOrgIdOrNull()` which is null during portal auth flow (ORG_ID not bound). Fix: thread orgId from PortalAuthController into PortalEmailService, resolve org name from OrganizationRepository. Grouped with GAP-PE-008. Spec: `fix-specs/GAP-PE-001+008.md` |
| GAP-PE-002 | Magic link URL in email points to frontend (:3000) not portal (:3002) | MEDIUM | SPEC_READY | -- | -- | **Bug.** `PortalEmailService` uses `appBaseUrl` (`:3000`) for magic link URL, should use `portalBaseUrl` (`:3002`). Also wrong path: `/portal/auth?token=` should be `/auth/exchange?token=...&orgId=...`. Config `docteams.app.portal-base-url` already exists in application.yml but is not injected into PortalEmailService. Spec: `fix-specs/GAP-PE-002.md` |
...
| GAP-PE-004 | Portal read model not populated on project creation — requires manual resync | HIGH | SPEC_READY | -- | -- | **Bug.** `ProjectService.createProject()` saves `CustomerProject` join record but does NOT publish `CustomerProjectLinkedEvent`. Only `CustomerProjectService.linkCustomerToProject()` publishes it. Proposal acceptance calls `ProjectService.createProject()`, so the event never fires and `PortalEventHandler` never syncs. Fix: add event publishing in `ProjectService.createProject()`. Spec: `fix-specs/GAP-PE-004.md` |
| GAP-PE-005 | Portal frontend has no proposals page — backend API exists but no UI | HIGH | SPEC_READY | -- | -- | **Feature gap.** Backend `/portal/api/proposals` (list, detail, accept, decline) all functional. Portal frontend has no proposals route. Fix: create proposals list page, proposal detail page with accept/decline, add nav link. Follows existing portal patterns (projects, invoices). Estimated ~1.5-2 hours. Spec: `fix-specs/GAP-PE-005.md` |
...
| GAP-PE-008 | Portal page title says "DocTeams" instead of firm name | LOW | SPEC_READY | -- | -- | **Bug (UX).** `portal/app/layout.tsx` hardcodes `"Client Portal | DocTeams"` in static metadata. Fix: change to `"Client Portal"` (remove "DocTeams"). Grouped with GAP-PE-001. Spec: `fix-specs/GAP-PE-001+008.md` |
