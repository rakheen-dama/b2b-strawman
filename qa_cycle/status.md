...
| GAP-D0-08 | Team member names display as "Unknown" instead of real names in mock-auth mode | LOW | SPEC_READY | Dev | — | Mock IDP JWT missing `name` claim. Fix bundled with GAP-D1-02 (adds `name` to token payload). |
...
| GAP-D1-02 | Bob (Admin) has degraded sidebar and pages crash — missing Clients, Finance, Court Calendar, Conflict Check sections | HIGH | SPEC_READY | Dev | — | Mock IDP JWT missing `role` claim. MemberFilter creates Bob as "member" instead of "admin". Fix: add `role` + `name` to JWT, read in MemberFilter. |
| GAP-D1-03 | Onboarding checklist item "Upload signed engagement letter" has requiresDocument constraint that cannot be satisfied — no documents on new client, Confirm silently fails | HIGH | SPEC_READY | Dev | — | **BLOCKER**: generic-onboarding pack item 4 has `requiresDocument:true`. Fix: change to `false` in pack JSON. Combined with GAP-D1-05 fix, legal-za gets FICA checklist instead. |
...
| GAP-D1-05 | Onboarding checklist is generic ("Generic Client Onboarding") instead of FICA-specific checklist for legal-za profile | MEDIUM | SPEC_READY | Dev | — | legal-za-onboarding pack has `autoInstantiate:false`, generic has `true`. Fix: swap both flags. Legal-za gets 11-item FICA checklist. |
