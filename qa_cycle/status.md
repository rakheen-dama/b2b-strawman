...
- **Day/Checkpoint**: Day 28 — checkpoint 28.1 (Days 0–21 complete, all PASS). Day 28 is `[FIRM]` — Bob generates first fee note (bulk billing wizard). **Pre-condition (OBS-2104)**: the Sheriff Fees disbursement (R 1 250, Draft/Unbilled on matter `66451e87…` Finance > Disbursements) must be Submitted for Approval + Approved before the wizard. Time entries: 4h total (2h30m + 1h30m), NO rate card → expect R 0 TIME lines (OBS-2101 path) unless rates set up first — prior cycle evidence in `_archive_2026-07-12…/checkpoint-results/day-28.md`. Bob's :3000 KC session live; Sipho :3002 portal JWT in localStorage (short expiry — re-request magic link at /login as needed). Court date 2026-07-26 Pre-Trial exists.
