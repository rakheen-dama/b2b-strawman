...
| GAP-DI-07 | Expired proposals can be accepted via portal | Major | OPEN | — | — | T1.3 | `PortalProposalService.acceptProposal()` checks status==SENT but does NOT check expiresAt. A proposal with expiresAt=2026-01-01 was accepted on 2026-03-24. Fix: add expiry check after status check in acceptProposal() (line ~126 of PortalProposalService.java). |
