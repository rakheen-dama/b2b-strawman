...
## QA Position
- **Day**: 7 — COMPLETE (pending gap triage for OBS-704)
- **Next checkpoint**: Day 8 (Sipho reviews + accepts proposal — portal)
...
| OBS-704 | Hydration mismatch on `/proposals` index page — CreateProposalDialog Radix DialogTrigger button has different aria-controls ID server vs client | MEDIUM | Dev | OPEN | 7 | Page renders correctly after client-side recovery. Radix ID generation differs between SSR and CSR. |
| OBS-705 | "Create Proposal" button text in engagement letter dialog should read "Create Engagement Letter" for legal-za terminology consistency | LOW | Dev | OPEN | 7 | Dialog title says "New Engagement Letter" but submit button says "Create Proposal". |
| OBS-706 | Engagement letter dialog Client combobox not pre-filled when accessed from org-level page | LOW | Product | OPEN | 7 | Scenario expected pre-fill from matter context. Product design is org-level dialog. |
| OBS-707 | Proposal email From address is noreply@docteams.app, not Kazi-branded | LOW | Dev | OPEN | 7 | From address uses legacy "docteams" domain. |
