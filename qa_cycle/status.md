...
| GAP-PE-003 | Raw JSON error shown on 404 portal project pages | LOW | FIXED | portal | PR #838 | **Bug (UX).** `portalGet()` and `portalPost()` in `portal/lib/api-client.ts` now return user-friendly messages for 404/403/generic errors instead of raw JSON. Project detail page error state replaced with centered "Project not found" layout with back link. 3 files changed. Lint, build, 87 tests pass. |
...
| GAP-PE-007 | No email notifications for proposal send/accept/decline | MEDIUM | FIXED | backend | PR #837 | **Bug.** `NotificationEventHandler.dispatchAll()` was passing `null` for recipientEmail, causing `EmailNotificationChannel` to skip all email deliveries. Now resolves recipient email from `MemberRepository` before dispatching. Note: email delivery still requires explicit `NotificationPreference` with `email_enabled=true` per type (opt-in by design). |
