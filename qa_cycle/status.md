...
| GAP-C-04 | D0 / 0.17 | HIGH | SPEC_READY | **Escalated MED → HIGH.** `TeamUtilizationWidget` is NOT a rate-dependency bug — root cause is `UtilizationService.getTeamUtilization()` calls `moduleGuard.requireModule("resource_planning")` and consulting-za profile has `enabledModules: []`. 403 cascades into dashboard server-action 500 on every render for every user. Breaks Bob/Carol dashboards right now AND breaks Day 75 wow moment. Spec: `qa_cycle/fix-specs/GAP-C-04.md`. Two-line JSON fix + one Flyway migration. | Dev | 0 |
| GAP-C-05 | D0 / 0.29 | HIGH | SPEC_READY | **Escalated MED → HIGH** (cascades into Day 34 profitability, Day 36 first-invoice, Day 75 wow moment). Root cause is two-pronged: (a) 8 seeded `billing_rates` rows have all 3 scope columns NULL (org-default) but `BillingRateService.resolveRate()` has NO `ORG_DEFAULT` branch — it only resolves PROJECT / CUSTOMER / MEMBER overrides. (b) The profile JSON's `rateCardDefaults` block already encodes Owner/Admin/Member 3-tier rates but `VerticalProfileRegistry` doesn't parse it. Fix: parse `rateCardDefaults` + seed MEMBER_DEFAULT rates on JIT member creation. Combined spec with GAP-C-06: `qa_cycle/fix-specs/GAP-C-05-C-06.md`. | Dev | 0 |
| GAP-C-06 | D0 / 0.31 | HIGH | SPEC_READY | **Escalated MED → HIGH** (same cascade as C-05). `cost_rates` empty because rate-pack seeder only writes billing_rates AND profile JSON's `rateCardDefaults.costRates` block isn't parsed. Same root cause + same fix as GAP-C-05 — combined spec at `qa_cycle/fix-specs/GAP-C-05-C-06.md`. | Dev | 0 |
