...
| OBS-4007 | Budget Alert automation SEND_NOTIFICATION fails: no PROJECT_OWNER recipients | LOW | Dev | FIXED | 30 | Two bugs: (1) `SendNotificationActionExecutor` queries `"LEAD"` (uppercase) but `Roles.PROJECT_LEAD` is `"lead"` (lowercase). (2) No fallback when project has no lead — returns empty list, causing ActionFailure. Fix: use `Roles.PROJECT_LEAD` constant + fallback to org admins/owners (mirrors `NotificationService.handleBudgetThreshold()`). PR #1311. Full verify: 5212 tests, 0 failures. |
