...
- **Day/Checkpoint**: Day 21 — checkpoint 21.1 (Days 0–15 complete, all PASS — isolation check clean, zero leaks). Day 21 is `[FIRM]` — Bob logs time (Phase A, non-tariff free-text), disbursement (Phase B, Phase 67), court date (Phase C) on matter `66451e87…` (RAF-2026-001). Clear cookies → login Bob (`bob@mathebula-test.local`). Probe IDs file retained for Day 46/75 spot-checks. Sipho :3002 session live (portal JWT in localStorage, exp ~22:37Z — re-request via magic link if expired).
