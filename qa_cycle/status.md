...
| OBS-4007 | Budget Alert automation SEND_NOTIFICATION fails: no PROJECT_OWNER recipients | LOW | Dev | SPEC_READY | 30 | Two bugs: (1) `SendNotificationActionExecutor` queries `"LEAD"` (uppercase) but `Roles.PROJECT_LEAD` is `"lead"` (lowercase). (2) No fallback when project has no lead — returns empty list, causing ActionFailure. Direct `NotificationService.handleBudgetThreshold()` succeeds via broader recipient set. Non-blocking — user receives notification via fallback path. Fix spec: `qa_cycle/fix-specs/OBS-4007.md`. |
| OBS-4008 | Budget Alert Escalation rule fails: Jackson null thresholdPercent deserialization | LOW | Dev | SPEC_READY | 30 | `common.json` `budget-alert-escalation` has `"triggerConfig": {}` (no thresholdPercent). `BudgetThresholdTriggerConfig(int thresholdPercent)` uses primitive `int` which cannot accept null. Jackson throws on `convertValue`. Fix: change to `Integer`, handle null as catch-all. Fix spec: `qa_cycle/fix-specs/OBS-4008.md`. |
