...
| GAP-D0-09 | Day 0 / V-D0-01 | HIGH | FIXED | Stale-org reuse path (GAP-D0-01 fix) creates tenant schema but does NOT seed `org_settings` with vertical profile, enabled modules, terminology, or pack statuses. Result: generic terminology, no legal modules, 500 errors on dashboard. Blocks entire Day 0 flow. **Fix**: pre-demo cleanup script (`compose/scripts/demo-cleanup.sh`) ensures clean state so the reuse path never fires. **FIXED via PR #1017**: script tested — deletes 4 KC orgs, 1 non-padmin user, truncates 4 tables, drops 4 tenant schemas, clears Mailpit. Idempotent (clean re-run verified). | Dev | 0 |
