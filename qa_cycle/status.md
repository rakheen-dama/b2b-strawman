...
- **Resolved**: none
- **Open gaps**: OBS-504 (LOW — activity feed misattributes info-request recipient to actor; SPEC_READY), OBS-505 (HIGH/BLOCKER, escalated from MEDIUM — seeded AI-specialist automations have wrong specialist IDs AND unresolvable `{{event.entityId}}` contextRef; fails on every envelope completion / invoice approval / matter reactivation; frontend dropdown mints broken rules too; SPEC_READY)
- **Fixed (awaiting verify)**: OBS-503 (MERGED to main, PR #1436 — awaiting QA retest via fresh REQ-0002 round-trip)
