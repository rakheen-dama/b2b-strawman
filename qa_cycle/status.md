...
| BUG-KC-003 | Keycloak user passwords not set during provisioning | MEDIUM | FIXED | Dev Agent | [#829](https://github.com/rakheen-dama/b2b-strawman/pull/829) | Auth | Fixed: added `setUserPassword()` to `KeycloakProvisioningClient`, called after invite in `AccessRequestApprovalService.approve()` (gated by `app.keycloak.set-default-password`). Bootstrap script backfills passwords for existing org members. PR #829 merged. Needs re-test in cycle 2. |
