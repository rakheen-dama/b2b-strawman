...
| GAP-PE-007 | No email notifications for proposal send/accept/decline | MEDIUM | SPEC_READY | backend | PR #837 | **Incomplete fix (v2 spec written).** PR #837 fixed email resolution in `dispatchAll()` but proposal handlers never call `dispatchAll()`. Three separate code paths bypass multi-channel dispatch: `onProposalSent()` uses `notifyAdminsAndOwners()` (in-app only), `ProposalAcceptedEventHandler` uses `createNotification()` directly, `ProposalService.declineProposal()` uses `createNotification()` directly. v2 fix: change `notifyAdminsAndOwners()` return type + add `dispatchAll()` calls + inject dispatcher into proposal handlers. Spec: `fix-specs/GAP-PE-007-v2.md` |
...
| GAP-PE-009 | Proposal acceptance crashes on null currency — invoice creation fails | HIGH | SPEC_READY | backend | -- | **Pre-existing bug.** `ProposalOrchestrationService.createFixedFeeInvoices()` passes `proposal.getFixedFeeCurrency()` to `Invoice` constructor. Field is nullable on Proposal but NOT NULL on invoices table. No send-time validation enforces currency presence for FIXED-fee proposals. Fix: (1) currency fallback to OrgSettings.defaultCurrency in orchestration, (2) send-time validation rejects FIXED proposals without currency. Spec: `fix-specs/GAP-PE-009.md` |
