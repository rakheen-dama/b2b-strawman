...
- **Next action**: Product — triage GAP-L-29-regression to SPEC_READY (root cause already identified by QA: `/api/customers` returns raw `List<CustomerResponse>`, frontend `fetchCustomers` in `frontend/app/(app)/org/[slug]/conflict-check/actions.ts:108-112` treats it as `PaginatedResponse`, returns `[]`). Per user-enforced no-workarounds rule, fix MUST land before QA resumes Day 2 — 2.6 (cannot continue past with L-28 unverifiable through UI).
...
| GAP-L-29-regression | Day 2 — 2.5 / 2.6 | MED (cascading on L-28) | OPEN | **NEW REGRESSION found in Cycle 1 verify on 2026-04-25 04:30 SAST.** Conflict-check form's Customer dropdown is empty because backend `/api/customers` returns raw `List<CustomerResponse>` (CustomerController.listCustomers line 92) but frontend server action `fetchCustomers` in `frontend/app/(app)/org/[slug]/conflict-check/actions.ts:108-112` treats response as `PaginatedResponse<>` and reads `result.content`. Result: `result?.content ?? []` always `[]`, dropdown never populates. Without customerId, conflict-check backend cannot self-exclude → L-28 fix's NO_CONFLICT outcome is unreachable from UI. **Blast radius**: blocks scenario 2.6 (CLEAR conflict-check outcome) and end-to-end UI verification of GAP-L-28. Does NOT block matter creation, documents, trust, billing (Day 3+). **Fix candidates**: (1) frontend one-line — `return Array.isArray(result) ? result : (result?.content ?? [])`; (2) backend wrap in `PaginatedResponse`; (3) dedicated `/api/customers/combobox` endpoint. Recommend (1) — minimal blast radius. Per user-enforced no-workarounds rule, MUST be fixed before QA resumes 2.6. | Product → Dev | 0 |
