...
|--------|------------------|----------|--------|---------|-------|---------|
| GAP-L-01 | Day 0 / 0.22 | HIGH | FIXED | Keycloak org-invite: same-profile session collision blocks registration AND single-use token is consumed on failed attempt, leaving user unable to register. Carry-forward of GAP-C-02; workaround of "fresh tab" does not work (tabs share cookies). **FIXED** via PR #1059 (squash-merged as `7758d6c3`): frontend `/accept-invite` bounce page routes invite click through KC's `end_session_endpoint` before landing on the invite action URL; email template (`org-invite.ftl`) rewritten to point at bounce page with original KC URL URL-encoded in `kcUrl`. | Dev | 0 |
