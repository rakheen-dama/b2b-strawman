...
| GAP-030 | Log Time crashes with RangeError: Invalid currency code null | blocker | OPEN | Dev | — | 7 | Clicking "Log Time" on any task crashes page with `RangeError: Invalid currency code : null`. The Log Time dialog tries to format billing amount using org currency, but org has no currency set (null). 100% reproducible. Blocks ALL time logging, billing, invoicing, profitability — Days 7-90. Related: GAP-008A (org settings "Coming Soon"), GAP-019 (currency display). Fix: seed default currency in org data OR add null-safe currency formatting. |
