...
| OBS-1001 | Trust deposit dialog: Popover combobox triggers (Client/Matter pickers) do not respond to clicks | MEDIUM | Dev | SPEC_READY | 10 | Triple Slot composition chain (`PopoverTrigger asChild` -> `FormControl` (cloneElement) -> `Button`) inside Radix Dialog. OBS-2103 bug class. Fix: remove `FormControl` from PopoverTrigger asChild chain in `TrustEntityPickers.tsx`. See `qa_cycle/fix-specs/OBS-1001.md`. |
