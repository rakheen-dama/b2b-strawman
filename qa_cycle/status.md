...
- **Cycle**: 1
- **Dev Stack**: READY
- **NEEDS_REBUILD**: true (GAP-C-04+C-07 fix PR #1055 + GAP-C-09(c) fix PR #1056 merged — backend restart required so V97 runs against existing consulting-za tenant to enable `resource_planning` + `automation_builder` modules, and so new `IntakeFieldResponse.visibilityCondition` DTO field is served by `/api/field-definitions/intake`)
...
| GAP-C-09 | D2 / 2.4, D5 / 5.7, D1 / 1.4 | MED | FIXED | Sub-issue (c) FIXED via PR #1056 (squash `3241a7bf`): `IntakeFieldResponse` DTO now preserves `visibilityCondition` and `from(FieldDefinition)` mapper copies it through. 4639 tests green. Awaits backend restart + QA re-verify at Day 1 checkpoint 1.4. Sub-issues (a) template `matterType` → project `campaign_type` auto-fill and (b) template budget defaults REMAIN DEFERRED per `qa_cycle/fix-specs/GAP-C-09.md` (need new DB columns + ~4–6h service work). | Dev | 0 |
