...
| GAP-PORTAL-02 | Project comments — 2 backend endpoints, 0 frontend | HIGH | FIXED | QA | #787 | PORTAL | Frontend code complete (PortalCommentSection with list+post). Was BLOCKED by empty portal read model. Fixed: added portal resync call to seed scripts (seed.sh Step 8 + rich-seed.sh final step). PR #787. NEEDS_REBUILD (seed change). |
| GAP-PORTAL-03 | Project tasks — 1 backend endpoint, 0 frontend | MEDIUM | FIXED | QA | #787 | PORTAL | Frontend code complete (PortalTaskList component). Was BLOCKED by empty portal read model. Fixed: same root cause as GAP-PORTAL-02. PR #787. NEEDS_REBUILD (seed change). |
| GAP-PORTAL-04 | Project summary/time tracking — 1 backend endpoint, 0 frontend | MEDIUM | FIXED | QA | #787 | PORTAL | Frontend code complete (3 summary cards). Was BLOCKED by empty portal read model. Fixed: same root cause as GAP-PORTAL-02. PR #787. NEEDS_REBUILD (seed change). |
...
| GAP-PORTAL-07 | Org branding on portal login page — 1 backend endpoint, 0 frontend | LOW | FIXED | Dev | #789 | PORTAL | Added `useSearchParams()` to read `?orgId` from URL and initialize `orgSlug` state. Wrapped component in `<Suspense>` (Next.js requirement). Fixed pre-existing test mock (added `useSearchParams`, corrected `orgSlug` -> `orgId` assertion). Frontend change — NEEDS_REBUILD. |
