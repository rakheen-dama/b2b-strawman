...
| OBS-1001 | Trust deposit dialog: Popover combobox triggers (Client/Matter pickers) do not respond to clicks | MEDIUM | Dev | OPEN | 10 | Triple Slot composition chain (`PopoverTrigger asChild` -> `FormControl` (renders Slot) -> `Button`) inside Radix Dialog. OBS-2103 bug class. Neither click, Space, Enter, nor force-click opens the Popover. Affects Record Deposit/Payment/Refund dialogs. Users cannot select Client or Matter in trust transaction dialogs. Fix: remove `asChild` chain per dialog-owns-button pattern. |
