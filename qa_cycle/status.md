...
| GAP-L-90 | BUG | OPEN | dev | Branding: brand-colour Save Settings persists `brand_color` to org_settings and injects `--brand-color: <hex>` on `<html>`, but no CSS rule consumes `var(--brand-color)`; setting brand colour has zero visible effect on UI. Logo persistence works fine. | qa_cycle/checkpoint-results/day-01.md §1.2; `day-01-1.2-after-refresh.png` |
| GAP-L-91 | LOW | OPEN | product | Legal-za rate pack ships LSSA 2024/2025 schedule, scenario 1.4 expects 2026. Either update scenario or seed a 2026 stub schedule. Functional intent (non-empty + ZAR + hourly entries) is satisfied. | qa_cycle/checkpoint-results/day-01.md §1.4; `day-01-1.3-1.4-tariff-schedule.png` |
