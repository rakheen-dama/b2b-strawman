...
## QA Position
- **Day**: 7 — COMPLETE (all gaps triaged: OBS-704/705/707 SPEC_READY, OBS-706 SCENARIO_AMEND)
...
| OBS-704 | Hydration mismatch on `/proposals` index page — CreateProposalDialog Radix DialogTrigger button has different aria-controls ID server vs client | MEDIUM | Dev | SPEC_READY | 7 | Fix: convert to dialog-owns-button pattern (eliminates SSR/CSR ID mismatch). Spec: `qa_cycle/fix-specs/OBS-704.md` |
| OBS-705 | "Create Proposal" button text in engagement letter dialog should read "Create Engagement Letter" for legal-za terminology consistency | LOW | Dev | SPEC_READY | 7 | Fix: wrap submit button text in `<TerminologyText>`. Spec: `qa_cycle/fix-specs/OBS-705.md` |
...
| OBS-707 | Proposal email From address is noreply@docteams.app, not Kazi-branded | LOW | Dev | SPEC_READY | 7 | Fix: change default sender-address from `noreply@docteams.app` to `noreply@kazi.app` in application.yml. Spec: `qa_cycle/fix-specs/OBS-707.md` |
