...
| GAP-PE-007 | No email notifications for proposal send/accept/decline | MEDIUM | REOPENED | backend | PR #837 | **Incomplete fix.** `dispatchAll()` now resolves recipient email correctly, but proposal event handlers (`onProposalSent`, etc.) call `notifyAdminsAndOwners()` which only creates in-app notifications — it never invokes `dispatchAll()` or `notificationDispatcher.dispatch()`. Email dispatch is bypassed entirely for proposal events. In-app notifications work; email does not. See `qa_cycle/checkpoint-results/portal-cycle3.md` for full root cause. |
