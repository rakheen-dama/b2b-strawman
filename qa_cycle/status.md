...
| GAP-DI-07 | Expired proposals can be accepted via portal (race window) | Major | FIXED | Dev Agent | [#833](https://github.com/rakheen-dama/b2b-strawman/pull/833) | T1.3 | SENT proposals with past expiresAt could bypass expiry guard before ProposalExpiryProcessor runs. Added expiry checks in PortalProposalService.acceptProposal() and declineProposal(), defense-in-depth guard in Proposal.markAccepted(), and isExpired() convenience method. 2 new integration tests. |
...
| 2026-03-24T22:41Z | Dev Agent | GAP-DI-07 FIXED. Closed race window where SENT proposals with past expiresAt could be accepted/declined before ProposalExpiryProcessor batch transition. Added isExpired() to Proposal entity, defense-in-depth guard in markAccepted(), expires_at to PortalProposalRow query, expiry checks in acceptProposal() and declineProposal(). 2 new integration tests (sentButExpired accept/decline). All 13 proposal tests pass. PR #833 merged (squash) into bugfix_cycle_financial_accuracy_2026-03-24. |
