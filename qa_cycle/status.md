...
| OBS-5003 | FICA checklist entity-type filtering broken — Phase 63 field promotion regression | MEDIUM | Dev | SPEC_READY | 2 | **Root cause: field promotion regression.** `ChecklistInstanceService.resolveEntityType()` reads `customFields.get("acct_entity_type")` but that field was promoted to a structural column (`Customer.entityType`) in Phase 63. The JSONB map no longer contains it, so `resolveEntityType()` always returns `null` and `isItemApplicable()` includes all items. Fix: read from `customer.getEntityType()` first, fall back to custom field for legacy data. Existing test masked the bug by manually injecting `acct_entity_type` into customFields. Effort: S (< 30 min). Fix spec: `qa_cycle/fix-specs/OBS-5003.md`. Non-cascading. **Day 4 confirmed**: PTY_LTD company also shows all 11 items with trust-specific items Required. |
| OBS-5004 | Trust field group auto-attached to non-trust clients — causes incorrect required field count | MEDIUM | Dev | OPEN | 4 | When creating a client with entity type PTY_LTD, the system auto-attaches both "SA Accounting — Client Details" AND "SA Accounting — Trust Details" field groups. Trust group has 3 required fields (Trust Registration Number, Trust Deed Date, Trust Type) irrelevant for company clients, causing "Required Fields: 2/5" in Client Readiness. The field group resolver (`FieldGroupResolver`) does not filter by entity type. Workaround: manually remove Trust Details field group via X button. Non-cascading. |
| OBS-5005 | Year-End Pack engagement template missing from accounting-za template pack | MEDIUM | Dev | OPEN | 6 | Template selector shows only 5 templates: Annual Trust Financial Statements, Monthly Bookkeeping, Payroll (Monthly), Tax Return — Company (ITR14), Tax Return — Individual (ITR12). Scenario expects "Year-End Pack" (15+ tasks) and scenario Day 0 checkpoint 0.41 also lists "VAT Return (bi-monthly)". The accounting-za template seeder does not include these. Blocks Day 6 checkpoints 6.2–6.6. |
