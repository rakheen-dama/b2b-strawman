...
- **Resolved**: None
- **Open gaps**: None
- **Fixed (awaiting verify)**: OBS-1001 (trust deposit dialog combobox triggers — FIXED, PR #1398 merged to bugfix branch)
...
| OBS-1001 | Trust deposit dialog: Popover combobox triggers (Client/Matter pickers) do not respond to clicks | MEDIUM | Dev | FIXED | 10 | Triple Slot composition chain (`PopoverTrigger asChild` -> `FormControl` (cloneElement) -> `Button`) inside Radix Dialog. OBS-2103 bug class. Fix: removed `FormControl` from PopoverTrigger asChild chain in `TrustEntityPickers.tsx` (both pickers). PR #1398 merged to bugfix branch. Frontend verified: 374 files, 2339 tests passed. |
