...
|-------|------|-------|---------|--------|
| 0a | 174 | 174A | `PaymentGateway` port interface + 6 value objects + `PaymentStatus` enum + `IntegrationRegistry.resolveBySlug()` | |
...
|-------|-------|---------|--------|
| **174A** | 174.1--174.7 | `PaymentGateway` port interface + `CheckoutRequest`, `CreateSessionResult`, `WebhookResult`, `PaymentStatus` enum + relocate `PaymentRequest`/`PaymentResult` to `integration/payment/` + `IntegrationRegistry.resolveBySlug()` method + unit tests. ~8 new files, ~1 modified file. Backend only. | |
