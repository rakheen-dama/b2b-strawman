...
| 3d | Epic 265 | 265B | **Header controls** — custom `KeycloakUserButton` + `KeycloakOrgSwitcher` in `auth-header-controls.tsx`. OrgSwitcher fetches from GET /api/orgs/mine, triggers re-auth with `kc_org` param. Update `sidebar-user-footer.tsx`. Depends on 264B + 263B. Can parallel with 265A. | **Done** (PR #516) |
| 3e | Epic 265 | 265C | **Team components** — update `member-list.tsx`, `invite-member-form.tsx`, `pending-invitations.tsx` with Keycloak branches (call backend API instead of Clerk hooks). Update `team/actions.ts` inviteMember server action for Keycloak. Depends on 264B + 263B. Can parallel with 265A, 265B. | **Done** (PR #517) |
...
| **265B** | 265.5-265.8 | Header controls: custom KeycloakUserButton (user info + sign-out), KeycloakOrgSwitcher (fetch from GET /api/orgs/mine, re-auth with kc_org param). Sidebar user footer | **Done** (PR #516) |
| **265C** | 265.9-265.13 | Team components: member-list, invite-member-form, pending-invitations — Keycloak branches calling backend API. Update team/actions.ts inviteMember for Keycloak | **Done** (PR #517) |
