...
| Order | Epic | Slice | Summary | Status |
|-------|------|-------|---------|--------|
| 4a (parallel) | 307 | 307A | `batchApprove()`, `batchSend()` with rate limiting, default due date/terms, progress tracking, approve + send endpoints, `BatchOperationResult`/`BatchSendRequest` DTOs. ~3 modified files (~8 tests). Backend only. | |
...
| 6b | 310 | 310A | Wizard scaffold (`billing-run-wizard.tsx`) with step navigation, Step 1 (`configure-step.tsx`), Step 2 (`customer-selection-step.tsx`), new billing run page route. ~6 new files (~6 tests). Frontend only. | |
...
| Slice | Tasks | Summary | Status |
|-------|-------|---------|--------|
| **307A** | 307.1--307.7 | `batchApprove()`, `batchSend()` with token-bucket rate limiting, default due date/terms application, progress tracking via `BillingRun.totalSent`, approve + send endpoints, `BatchOperationResult`/`BatchFailure`/`BatchSendRequest` DTOs. ~3 modified files + 1 new test file (~8 tests). Backend only. | |
...
| Slice | Tasks | Summary | Status |
|-------|-------|---------|--------|
| **310A** | 310.1--310.7 | Wizard scaffold with step navigation (`billing-run-wizard.tsx`), Step 1 configure (`configure-step.tsx`), Step 2 customer selection (`customer-selection-step.tsx`), new billing run page route. ~6 new files (~6 tests). Frontend only. | |
