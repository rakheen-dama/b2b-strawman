...
| 1d | Epic 533 | 533B | Integrate `GroupedTabBar` into `ProjectTabs`, replacing flat tab triggers. Depends on 533A. | **Done** (PR #1342) |
...
| **532B** | 532.5-532.10 | `MatterSidebar` component with identity, metadata, custom fields (accordion), tags, sticky footer placeholder. Refactor `page.tsx` to wrap in `MatterDetailLayout` and distribute data to sidebar vs main. Unit tests (~6 tests) | **Done** (PR #1340) |
...
| **533B** | 533.6-533.8 | Integrate `GroupedTabBar` into `ProjectTabs`, replacing flat tab triggers. Keep all content panels. Update `ProjectTabs` props interface if needed. Integration tests (~4 tests) | **Done** (PR #1342) |
