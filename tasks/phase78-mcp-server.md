...
|-------|-------|---------------|---------|
| **562A** | 562A.1–562A.3 | ~4 backend files (1 `pom.xml` modify + 1 config + 1 security config modify + 2 yml modify) | Spring AI 2.0 milestone starter + STREAMABLE protocol; `McpServerConfig` (identity, version, read-only instructions, advertise `resources`+`tools`); Spring Security wiring so `/mcp` runs the full filter chain. | |
